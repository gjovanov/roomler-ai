 ...
 * resume. Field repro on CORPLAP-1 2026-05-11/12: large file uploads
 ...
 * field; field repro on CORPLAP-1 made it clear that operators on
...
export function useRemoteControl(agent?: Ref<Agent | null>) {
  ...
  function fetchAgentLogs(lines = 500): Promise<RcLogsFetchReply> {
    return new Promise((resolve, reject) => {
      ...
      let isActive = true
      // rc.23 hotfix #2 — 30 s timeout (was 8 s). CORPLAP-1 field
  ...
  async function innerPump(file: File, startOffset: number, id: string): Promise<void> {
    // rc.23 hotfix #3 (CORPLAP-1 2026-05-13): 64 KiB sat exactly at
   ...
   * tab. Field repro on CORPLAP-1 2026-05-12 (CV.pdf upload + rc.23
  ...
  function uploadOne(
    ...
    return new Promise((resolve, reject) => {
      ...
      // no-op. Was 6; rolled forward on CORPLAP-1 field repro where
      ...
      const runResumable = async (): Promise<void> => {
        while (attempt < MAX_ATTEMPTS) {
          try {
            ...
            // observed during auto-update on CORPLAP-1). Outer
            ...
            // repro on CORPLAP-1 2026-05-12 (CV.pdf upload). Backstop
