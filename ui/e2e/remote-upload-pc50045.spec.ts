/**
 * E2E: upload a >1 MB file to CORPLAP-1 via the live roomler.ai
 ...
 *  - CORPLAP-1's agent is online and registered as
...
test.describe('Remote upload to CORPLAP-1', () => {
  test.skip(
  ...
  test('upload CV.pdf to CORPLAP-1 and wait for completion', async ({ page }) => {
    ...
    if (lastStatus !== 'complete') {
      ...
      throw new Error(
        `Upload did not complete within 12 min. Last status: "${lastStatus}". ` +
          'Check the Transfers panel + agent log on CORPLAP-1.'
      )
    }
