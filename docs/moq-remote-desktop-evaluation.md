...
- **P2P WebRTC** (webrtc-rs): the controlled-host **agent is the answerer**, the **browser controller is the offerer**; media flows agent↔browser directly, **TURN-relayed only when NAT requires** — and TURN relays *opaque DTLS*, so **the server never sees pixels**. (`agents/roomler-agent/src/peer.rs`; `crates/remote_control/src/turn_creds.rs`.)
...
- **Publisher module** in `agents/roomler-agent`: clone `media_pump_vp9_444_dc` (`peer.rs:1634`) into `media_pump_moq` — identical capture/keyframe/AIMD/scene-change logic, only the *sink* differs (publish object instead of `dc.send`). Reuse `frame_video_bytes` **verbatim**.
...
- `agents/roomler-agent/src/peer.rs` — publisher source to clone (`media_pump_vp9_444_dc:1634`, `frame_video_bytes:1605`, dispatch `:885-987`).
- `agents/roomler-agent/src/signaling.rs` — agent-side `rc:broadcast.*` handling + a new broadcast-peer map (mirror the `TunnelQuicSetup` arm + `close_all_tunnel_quic_peers` teardown).
...
- `agents/roomler-agent/src/encode/caps.rs` — probe-gated `"moq-broadcast"` cap + a default-build-omits-it test.
