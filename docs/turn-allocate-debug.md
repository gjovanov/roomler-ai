...
Field repro 2026-05-17 from CORPLAP-3 / CORPLAP-2 agent logs:
...
**(b)** A corporate middlebox between CORPLAP-3 and coturn
...
1. **Packet capture on CORPLAP-3** via Wireshark / `pktmon` for
...
(1) is the right next step — needs a person at CORPLAP-3 to run
