...
| P2c | remaining cheap classes: log filenames ✅, install/staging paths, e2e image, wizard PATH | per-item revert | `TermsView` ✅, **logs ✅**; e2e image + wizard PATH open |
| P3 | appdirs trees, Windows service + task, install folder | staged rollout + rollback build | **fleet side DONE** — no legacy tree on any reachable host; the code dual-read is now dead weight |
