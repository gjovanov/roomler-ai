...
Status: **P1 COMPLETE and field-verified (0.4.26 + web `v20260830-6a166a1ec9f7`; five rotations on CORPLAP-3, one on CORPLAP-2, every one clean; P1b/P1c/P1d closed the report/redelivery defects the field runs found); P2 (retired keys) and P3 (CLI) open** (2026-08-30). Tracking issue: `FR-40` (#962).
...
- [ ] a device that is offline is `queued`, and rotates on its next connect with no operator action — integration-tested (queued); the connect-time delivery itself was exercised by the third cycle (an order delivered on connect at 01:30:36 and 01:34:54), not by an offline device
...
- [ ] `overlay_key_rotation = false` on the device ⇒ `refused: disabled`, key unchanged — unit-tested (config surface); not field-verified (needs a daemon restart on a corp laptop for a kill switch)
