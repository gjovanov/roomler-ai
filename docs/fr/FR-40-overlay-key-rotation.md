...
Status: **P1 shipped (0.4.25/0.4.26 + three API deploys); three field cycles on CORPLAP-3 — the rotation works every time; three report/redelivery defects found and fixed (P1b, P1c, P1d); P2/P3 open** (2026-08-30). Tracking issue: `FR-40` (#962).
...
| P1c | the join is the proof: the order snapshots the device's public key and the state resolves `rotated` from a verified identity change (report or no report — the report rides the dying session and was lost in run 2); the agent re-sends its `rotated` report on the next session, per org | — | implemented, PR pending; agent half ships in 0.4.26 |
| P1d | an order the device has already executed (its join under a key ≠ `public_key_before`) is satisfied and never re-delivered — one click, one rotation, whatever became of the report | — | implemented, PR pending |
