...
| **P5** | Export engine and the Edit view; the vmtest `record` cell | Edit hidden when `roomlerd media --version` fails | **P5a built** (`docs/recording.md` §11): the edit list (`recording/edit.rs`: contiguous keep / cut / speed segments, speeds 1.25–16 in quarter steps, refused by name) and its exact integer time map; the export engine (`recording/export.rs`: openh264 decode forward with keyframe jumps across cuts, the recording encoder, the partial locked like a recording's, nothing left behind on failure or cancel); `roomlerd media probe\|export`; `tests/export.rs` — the plan's oracle decodes to exactly `0..59, 120…236, 240..299` (red with the map ignoring speed, the frame choice off by one, the cancel check removed, the name check removed). **P5b built**: the recording's own sound through the same plan (muted under a speed-up, gone with a cut) and background music (`recording/export_audio.rs`: symphonia reads MP3, AAC/M4A, FLAC, Ogg Vorbis and WAV, streamed and resampled by the recorder's resampler; volume, start, fade in and out, loop), mixed through the recorder's soft clip into one Opus track; the `done` event says what the sound carries (`none`, `original`, `music`, `original_and_music`, `not_carried`); `tests/export.rs` `mod sound` reads a stepped tone back second by second (red with a speed-up not muted, the cut ignored, no loop, no fade, the volume ignored). The music file is untrusted input: a fuzz pass over 1,800 malformed files found a WAV declaring 0 Hz that panics symphonia 0.5.5's probe, so every call into it is guarded and a rate outside 1–768 kHz is refused `music_unreadable` (red with the guard removed, the range widened). Open: **P5c** the Edit view, FFmpeg decode for hardware recordings and AAC after **P4** |
