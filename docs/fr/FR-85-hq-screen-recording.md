...
| **P5** | Export engine and the Edit view; the vmtest `record` cell | Edit hidden when `roomlerd media --version` fails | **P5a built** (`docs/recording.md` §11): the edit list (`recording/edit.rs`: contiguous keep / cut / speed segments, speeds 1.25–16 in quarter steps, refused by name) and its exact integer time map; the export engine (`recording/export.rs`: openh264 decode forward with keyframe jumps across cuts, the recording encoder, the partial locked like a recording's, nothing left behind on failure or cancel); `roomlerd media probe\|export`; `tests/export.rs` — the plan's oracle decodes to exactly `0..59, 120…236, 240..299` (red with the map ignoring speed, the frame choice off by one, the cancel check removed, the name check removed). Video only, and says so (`audio: not_carried`). Open: **P5b** the original audio through cuts and speed-ups plus music, **P5c** the Edit view, FFmpeg decode for hardware recordings after **P4** |
