...
**Issue:** [#1634](https://github.com/gjovanov/roomler-ai/issues/1634) · **Status:** **in progress** — P0's FFmpeg half measured (field log); P1a (the recorder core), P1b (the hardware recording profile), P1c (audio on Windows and Linux), P2a (the local verbs: LocalAPI, console-user gate, `record_dir`, `roomler record`) and P2b (roomler-desktop's Recordings view and tray) built; P3a (the server's gates for remote recording), P3b (the device's half: the owner's gates, the `record` DataChannel, the host prompt, the indicator before the first frame), P3b-2 (the download) and P3c (the viewer) built; P1e (the recorder runs as the person at the device) built on Windows; `recording` joined `full` with every gate closed and `ROOMLERD_RECORDING=0` as its kill switch (#1677), and was taken back out before any release carried it (the operator, 2026-09-26): it rejoins after canary field checks · **Numbering:** claimed as FR-84 and renumbered before landing — [#1633](https://github.com/gjovanov/roomler-ai/issues/1633)'s FR-84 reached master first (issues 68 s apart), and the ledger arbitrates · **Anchors:** master `d5db02c0f` · **Glossary:** [`CONTEXT.md`](../../CONTEXT.md) · **Related:** [FR-27](FR-27-consent-surfaces-and-desktop-companion.md) · [FR-44](FR-44-shared-pipeline-multi-viewer.md) · [FR-70](FR-70-media-pipeline-leaves-the-runtime.md) · [FR-77](FR-77-encoder-chroma-matrix.md) · [FR-80](FR-80-a-session-with-no-pixels-says-why.md) · [FR-24](FR-24-licensing-split.md) · [FR-61](FR-61-vmtest-matrix.md) · [`docs/remote-control.md` §11.3](../remote-control.md)
...
| Question | Decision |
|---|---|
| Does a host at its **login screen** count as "nobody signed in" for an unattended recording (open decision 6)? | **No.** A remote recording there is refused, by its own name (`login_screen`), before anyone is asked. The unattended exception stays only for a host with no screen anyone could be at (no active graphical session) and for a host running its own virtual desktop. On Windows that is never: the console shows the sign-in screen whenever nobody is signed in. |
| **P2c** — the Linux user unit cannot write into `~/Videos` | The cheap fix: `ReadWritePaths=-%h/Videos` in the user unit. Any other folder keeps its recordings in the data folder, and the Recordings view names the reason. A localized XDG Videos folder falls back to the data folder. The companion-side mover is built only if people ask for it. |
| **P4** — the FFmpeg bump (software decoders, AAC) | Yes, and it ships **with the recorder**: in the release where `recording` rejoins `full`, not before. H.264-only decoding (about −0.4 MiB) is worth considering (open decision 2). AAC is mainly for playback in Apple's players and QuickTime. `lgpl-source-offer.yml` is dispatched before that tag. |
| How do **canary** recorder builds reach hosts? | A separate test build (`full` + `recording`), installed only on the EDR-managed laptop and the Mac, never on the fleet's update channel: through a pre-release channel if the updater has one, otherwise by hand. EDR: open the Recordings view, make a local recording, then a remote one, and watch for alerts or quarantine. Mac: the Screen Recording and Microphone permissions work, and existing grants survive the update. **Settled (the operator, same day):** there is no pre-release channel. The updater and `/api/agent/latest-release` accept pre-releases, so anything published reaches the whole fleet. The canary is therefore:<br>- **version `0.4.106`, reserved:** never tagged; the next fleet release is 0.4.107 (the `ship-it` skill);<br>- from branch `fr85-canary-0.4.106`, never merged, which adds `recording` to the Windows build line only;<br>- a signed `release-agent.yml` dispatch with `publish_release=false`;<br>- installed by hand on one EDR laptop (both corporate laptops run Microsoft Defender for Endpoint and the per-machine service), with auto-update off.<br>**The Mac half is blocked:** the operator's Mac runs a root daemon, which refuses recording (`RootDaemon`), and a session delegated to its GUI worker has no record context. So the recorder cannot run there until the macOS drop is built (P1e). |
...
| `symphonia` (MP3, AAC/M4A, FLAC, Ogg Vorbis, WAV) and the export's sound (P5b) — **measured in P5b**; no `rubato` (the recorder's own resampler serves) | roomlerd, all platforms | **+1.38 MiB** linked (Windows release, one codegen unit: 35,525,120 → 36,972,544 B); +1.10 MiB with music limited to MP3, AAC/M4A and WAV | ≈ +0.48 MiB (at the MSZip ratio; the P4 release run measures it) |
...
| **Total** | | **≈ +5.1–5.5 MiB** | **≈ +1.5–1.7 MiB per installer (≈ +10–11 %)** |

- **Windows MSI ≈ 17.0 MiB** (15.36 at 0.4.101 + 0.82 for FFmpeg, measured + ≈ 0.48 for P5b, measured linked and
  compressed at the MSZip ratio + ~0.35 estimated for the rest), under the 18 MiB ceiling with about 1.0 MiB to spare.
  P5b came in above its estimate (≈ 0.48 against 0.3). P4 re-pins the baseline **from the measured release run, never
...
- Levers if the ceiling gets tight: H.264-only editing (drop the `hevc` decoder, ≈ −0.4 MB) · MP3/WAV/M4A-only music
  (measured: −0.28 MiB linked, ≈ −0.1 MiB in the installer).
...
| **P1** | Recorder core behind cargo feature `recording` (joins `full`): the child and its identity rule, own capturer + cursor, CFR pacer, `RateMode`/GOP profile, fMP4 + remux + reconciler, audio (system + microphone → Opus), sidecar, guards, write rule. macOS: `NSMicrophoneUsageDescription` (the `Info.plist` heredoc, `release-agent.yml` ~:2417) and the `com.apple.security.device.audio-input` entitlement — neither exists, and `docs/remote-control.md:923-925` wrongly says the entitlement is covered. The designated requirement does not change, so existing Screen Recording grants must survive; verify on the MacBook, because FR-80 was a TCC break. | `ROOMLERD_RECORDING=0` (built with the ship step: every path answers "switched off", the device stops advertising `record`); out of `full`, and so out of every release build, until canary hosts prove it in the field (the operator, 2026-09-26; it joined `full` for a day with #1677) | **P1a shipped**: `roomlerd record`, own capturer, CFR pacer + TickFifo, openh264 recording profile (HW backends: the live cascade with a forced GOP), fMP4 + remux + reconciler, folder rules, sidecar, the prefixed JSON-lines protocol; `tests/recorder.rs` named in CI; `docs/recording.md`. **P1b shipped**: `FfmpegEncoder::new_recording` — the live option sets (field-proven) with `cq` 19, a recording ceiling (NVENC ~0.3 bpp·s as a burst cap over CQ; the maxrate-anchored backends ~0.12 bpp·s as the bitrate), a real GOP and counter-based PTS; `hardware` refuses rather than fall back to software; verified locally by `ffprobe` on `h264_nvenc` (High, BT.601 tagged, keyframes at 0/60, zero decode errors). **P1c built (Windows, Linux)**: computer audio (loopback / monitor ONLY, never the Linux fallback to a microphone) and the microphone, both off unless asked for, mixed on the recorder's clock with a 60 ms lag, drift corrected by rate (±0.5 %), soft-clipped, Opus 128 kb/s in the MP4, aligned to the first written video frame; each source asked for and not opened refuses the start by name (`docs/recording.md` §9). Open: true per-backend CQ modes (QSV ICQ, AMF/VAAPI CQP) after field measurement, **P1c-mac** the microphone on macOS (cpal there, `NSMicrophoneUsageDescription`, the `audio-input` entitlement) and computer audio there (ScreenCaptureKit, macOS 13+). **P1d built** (`docs/recording.md` §1 "The pointer"): the mouse pointer is in a recording wherever the platform allows it. WGC's recording session draws it (`capture::open_for_recording`; the live session still does not), and CoreGraphics and an embedded portal cursor already did. On DXGI and X11 the recorder draws it itself, on its own tick, from `GetCursorInfo` and XFixes placed by the display's origin (`recording/pointer.rs` `PointerLayer`: the covered pixels put back, `make_mut` so the encoder's frame is never written, the frame handed over owned so nothing is copied). The sidecar says how (`in_frame`, `drawn`, `none`). CI compiles the Windows capture code (WGC, DXGI, the cursor tracker) for the first time, and runs the X11 source against Xvfb. Open: DRM and the SystemContext capture record no pointer; the field cells (P6). **P1e built on Windows** (`recording/launch.rs`, `docs/recording.md` §6 "Who the recorder runs as"): an elevated worker launches the recorder with a restricted copy of its OWN token (admin-equivalent groups deny-only, `DISABLE_MAX_PRIVILEGE`, medium integrity, owner and default DACL the user's; the linked token is identification-only without `SeTcb`); a SYSTEM worker with someone signed in uses their `WTSQueryUserToken`; both through `CreateProcessAsUserW` with a bounded handle list, the recorder's stderr into the daemon log; the daemon's list, delete and download-open run impersonating the same token; the folder is asked of the recorder (`record --where`, cached a minute); `record --whoami` reports the identity; a new elevated Windows CI lane. Measured here: an elevated daemon (integrity 12288, admin enabled) launches a recorder at 8192 with the admin group deny-only, the same user. **P1e-unix built (Linux)**: a root daemon launches the recorder as the person at the active graphical session (`Class=user`, never a greeter; uid 0 counts as nobody) through the one privilege drop (`exec::drop_to_std`) with that session's environment and X cookie, and does its own folder work with the thread's fsuid, fsgid and supplementary groups switched to theirs (`launch::unix::FsIdentity`: the per-thread `setgroups` syscall, every switch verified, a failed restore aborts); a privileged CI step runs those lib tests as root. **P1f built** (the unattended exception): a SYSTEM or root daemon with nobody signed in records a REMOTE session as itself (`Identity::Unattended`) into its own folder, locked to the service side (a protected DACL of SYSTEM and Administrators in `%PROGRAMDATA%` on Windows, 0700 elsewhere; a link on the path refused; never `record_dir`), with no banner because there is nobody to show one to, the controller told `unattended: true`; the identity is decided once, the launch refused if someone signed in meanwhile, and a running unattended recording stopped `session_changed` the moment someone signs in (all asked fresh, never from the 5 s cache, a review finding); a local recording there is still refused; lists and downloads search both folders; `Dockerfile.agent-e2e` carries `recording`. **Decision 6 built** (2026-09-26): a login screen is not "nobody". Windows' sign-in screen, a Linux display manager's greeter and root's own desktop refuse every start `login_screen`, with the words, a remote one before the host is asked; on Linux the unattended exception now needs the host's own virtual desktop (`virtual_desktop::requested`, now the one copy) or the synthetic test source, or else no active graphical session AND no real screen the recorder could reach — no `DISPLAY`/`WAYLAND_DISPLAY` in the daemon's environment, no DRM scanout (`launch::unix::Seat` `Person`/`NoPerson`/`Empty`/`Unknown`, `Reach`, `shows_a_screen`; from an independent review: "no logind session" is not "no screen", and a `loginctl` that cannot be asked is not "no session"); a Windows service never records unattended; an unattended recording stops `session_changed` when a login screen comes up, and a login screen found at the launch re-check refuses by its own name; the device keeps advertising `available` (a capability, not who is at the screen) and a login screen keeps what was recorded listable. Open: the drop on **macOS** (a root daemon there still refuses), **P1e field** the SystemContext (`ConsoleUser`) path and a real Linux desktop (X11, and GNOME Wayland, where the recorder's own ScreenCast session may make the portal ask the person first) |
| **P2** | Local: `record_*` keys, validators and the `DesiredConfig` prefix exclusion; LocalAPI verbs with the session peer check; the Recordings view, settings, folder picker and tray REC; the CLI; updater defer; delivery from the data dir | roomler-desktop greys Start out, with the reason, against a service without the recorder or one that cannot record here (`RecordingState.available`), and says "no screen recorder yet" to one that predates the verbs | **P2a built**: `record_dir` (live, one shared validator, structurally absent from `DesiredConfig`); the five LocalAPI verbs behind the console-user gate in the listener; `RecordingManager` (spawn, event folding, one at a time, `recorder_exited`); the SYSTEM/root refusal until P1e; reconcile at every recorder start, guarded by `PartialLock`; updater defer (`active_work`); `roomler record start\|stop\|status\|ls\|rm`. **P2b built**: roomler-desktop's Recordings view (start/stop and options, the folder with Change/Default/Open, the list with Play/Show/two-click Delete), the native folder picker, the tray's Start/Stop item and tooltip, `available`/`unavailable_reason` on the state, a daemon answer final for `record_*` keys, the companion's unit tests in a CI lane at last. **P2c built** (the cheap fix, the operator's choice of 2026-09-26): the Linux user unit (`packaging/linux/roomler.service`) lists `~/Videos` as writable (`ReadWritePaths=-%h/Videos`), so the recorder's default folder works under its `ProtectHome=read-only`; a folder anywhere else under home is refused by the test write, falls back, and the Recordings view names the sandbox as the cause (`folder::sandboxed`); a localized Videos folder falls back the same way; the companion-side mover is not built (only if people ask). Found on the way and fixed in the same line: the unit listed the pre-rename folders WITHOUT systemd's `-`, so on a host that never ran the pre-rename agent it failed to start (`226/NAMESPACE`) wherever the sandbox applies — the default `install.sh --role daemon` path. It applies only where the user manager may mount inside an unprivileged user namespace. Where it cannot create one (measured, systemd 255 with `user.max_user_namespaces=0`), or may not mount in it (default Ubuntu 24.04, AppArmor's userns restriction: field-verified by the release session on a fresh vmtest guest, where the daemon shared the session's mount namespace and the unit started without the pre-rename folders), systemd runs the unit with no sandbox at all, so there `~/Videos` was writable all along. A CI step now starts a unit carrying the shipped sandbox lines on a fresh runner and asks the real recorder where it would write. **P2d built**: the tray turns red while any recording runs, this device's own or a controller's, from the 750 ms poll the banner rides as well as its own; only an answer moves it; the red icon is never a macOS template (`tray.rs`, `icons/tray-recording.png`, `docs/recording.md` §7) |
| **P3** | Remote: role bit, `may_record`, hub strip (break-glass included), `AgentCaps.record`, the `record` DataChannel, `PromptKind::Record`, indicator before the first frame, grace re-attach, list/get with resume, `RecordingActivity` + collection + baseline, viewer UI | device gate default **OFF**; the role bit granted to nobody by default; the `remote` module switch makes the hub strip RECORD | **P3a built** (the server's half): `RECORD_REMOTE_SCREEN` (bit 31, `ALL` bumped, in no managed row below ADMINISTRATOR, the UI mirror `2 ** 31`); `SessionAuthz.may_record` (owner, ADMINISTRATOR, or the bit; **false under break-glass**), carried across pods by `relay_rc_frame`; `record_grant` in the hub, after the INPUT rule, naming `controller_not_allowed` / `device_not_opted_in` in `SessionCreated.record_refused` and the audit's `SessionRequested.record_stripped` (a coalesced duplicate request repeats its session's reason); `AgentCaps.record` + `RecordCap` (`remote`, `remote-audio`, equality), kept per connection and refreshed from a heartbeat that re-announces caps, so the owner's ON/OFF needs no reconnect; `rc:recording.activity` → `recording_activity` (TTL 90 d), kept only for a session of the sending device whose effective grant held RECORD — the live one, or its stored row once it has ended; `GET …/recording-activity/{agent_id}` under `VIEW_REMOTE_AUDIT`; `crates/tests` `remote_recording_tests` (`docs/recording.md` §10). **P3b built** (the device's half): `record_remote_enabled` / `record_remote_audio` (device-only, live, console-gated; an OFF stops a remote recording `gate_revoked`); `AgentCaps.record` advertised only while the switch is on AND a recorder can run in the process, re-announced on the heartbeat when it changes (the announcement is now built like the hello, so it no longer drops `multi_org` `tun`); the `record` DataChannel behind the attach-time RECORD gate (`not_granted` otherwise); a start refused by name in order (`disabled_on_device`, `unavailable`, `audio_not_allowed`, `busy`, `already_starting`); a host-consented session asks again with `PromptKind::Record` on a FRESH prompt id, a deny holding 60 s (`rate_limited`); the indicator before the first frame (the Windows badge pinned open and relabelled; the companion's banner with **Stop recording**), or `no_indicator_surface`; `RecordingManager::start_remote` (never the microphone), stops named `requested` / `host_stopped` / `gate_revoked` / `session_ended`; every outcome reported as `rc:recording.activity`; roomler-desktop's two switches; `tests/control_dc_record.rs` named in CI. **P3b-2 built** (the download): `rc:record.list` (this controller's recordings, by the sidecar's `controller_user_id`, so a flap's new session id still reaches them) and `rc:record.get {name, offset}` (a bare `*.mp4` name, opened without following a link, someone else's recording answered `not_found` like a missing one; the files channel's pacing; resumable by offset with no size cap; `done` carries the WHOLE file's SHA-256, the prefix hashed from disk; one transfer per session; `downloaded` reported). **P3c built** (the viewer): RECORD asked for on every session, the toolbar reading the grant (a Record menu with the `record` channel; a stripped grant a disabled control whose tooltip names why; an older server, nothing); `useRemoteRecording` (start with computer audio as a separate unticked box and no microphone offer, the REC timer, every refusal and ending one sentence, the device's list, a download streamed into a picked file or held in memory up to 2 GiB and kept only when its SHA-256 matches, a short file an error, resumed on the reconnect ladder's next `record` channel); unit spec plus two agent-lane e2e specs. **P3c-2 built**: a device with no recorder is told apart from one whose owner has not opted in. The agent advertises `available` whenever a recorder can serve a remote session, `remote` only with the owner's switch on as well; the hub's `RecordSupport` (none, not opted in, serves) names `device_cannot_record` first, whoever asks; the viewer shows no Record control for it (`showsRecordRefusal`). Without it, a server promote would have shown every connected session a disabled Record control saying the device's owner had not allowed recording, on a fleet where no device can record. **P3b-3 built** (`docs/recording.md` §10 "A dropped session"): a remote recording outlives its session for `REATTACH_GRACE` (60 s) instead of ending `session_ended` at once. The session's banner is KEPT, marked `reconnecting` (`ViewerIndicator::hide_session` keeps a recording session's entry; `RcSessionInfo.reconnecting`; the companion keeps Stop and drops Disconnect). The SAME controller's next session picks it up when its `record` channel asks for the status (`retarget_remote`, the new banner first, then the old one down, `reattached` reported). Nobody else can. The grace still stops it for the owner's OFF and, unattended, for a sign-in; otherwise it ends `session_ended` when the grace runs out, reported once on the dropped session. A session's end is the signalling loop's word (`remote::session_ended`, before the bounded peer close) as well as the channel's state (`Closing` or `Closed`). **The host's Disconnect STOPS the recording** (`remote::host_ended`, `host_stopped`), and the viewer's Disconnect stops it first. The grace acts on its own recording only and claims it before stopping it; one speaker per recording, keyed by its file (all four from an independent review). The viewer shows REC · reconnecting and adopts the device's recording id. The red tray icon: P2d |
| **P4** | One vendored-FFmpeg bump: `--enable-decoder=h264,hevc --enable-encoder=aac` on Windows (`vendor-ffmpeg-windows.yml` `$inject`), Linux (its configure + `probe.c`) and macOS (`vendor-ffmpeg-macos.yml`); invert the absence assertions (`ff_h264_decoder` becomes required, `ff_mov_muxer` stays forbidden); update `THIRD-PARTY-NOTICES.md`, `docs/encoders.md`, `docs/lgpl-relink.md`, `docs/fr/FR-24-*.md`; dispatch `lgpl-source-offer.yml` before tagging; re-pin the MSI baseline; AAC becomes the audio codec (arm64 keeps Opus) | a new asset suffix flipped in `release-agent.yml` (rollback = flip back) | **Decided 2026-09-26**: ships with the recorder, in the release where `recording` rejoins `full` (never before it); `lgpl-source-offer.yml` dispatched before that tag; H.264-only decoding weighed then (open decision 2). Not built |
| **P5** | Export engine and the Edit view; the vmtest `record` cell | the Edit button shows only where `roomlerd media --help` succeeds (`cmd_media_available`) | **P5a built** (`docs/recording.md` §11): the edit list (`recording/edit.rs`: contiguous keep / cut / speed segments, speeds 1.25–16 in quarter steps, refused by name) and its exact integer time map; the export engine (`recording/export.rs`: openh264 decode forward with keyframe jumps across cuts, the recording encoder, the partial locked like a recording's, nothing left behind on failure or cancel); `roomlerd media probe\|export`; `tests/export.rs` — the plan's oracle decodes to exactly `0..59, 120…236, 240..299` (red with the map ignoring speed, the frame choice off by one, the cancel check removed, the name check removed). **P5b built**: the recording's own sound through the same plan (muted under a speed-up, gone with a cut) and background music (`recording/export_audio.rs`: symphonia reads MP3, AAC/M4A, FLAC, Ogg Vorbis and WAV, streamed and resampled by the recorder's resampler; volume, start, fade in and out, loop), mixed through the recorder's soft clip into one Opus track; the `done` event says what the sound carries (`none`, `original`, `music`, `original_and_music`, `not_carried`); `tests/export.rs` `mod sound` reads a stepped tone back second by second (red with a speed-up not muted, the cut ignored, no loop, no fade, the volume ignored). The music file is untrusted input: a fuzz pass over 1,800 malformed files found a WAV declaring 0 Hz that panics symphonia 0.5.5's probe, so every call into it is guarded and a rate outside 1–768 kHz is refused `music_unreadable` (red with the guard removed, the range widened). **P5c built**: roomler-desktop's Edit view (`src/front/editor.js`, `src/editor.rs`, `docs/recording.md` §11 "The Edit view"): the timeline with split, keep, cut and speed up (a split stays the person's), the sound card with music, the export with progress, cancel and the result named with its sound; every change saved as the edit list roomlerd reads, one shared fixture pinning both ends (`agents/roomler-desktop/tests/fixtures/edit-list.json`); `roomlerd media` run as the person, one export at a time, a silent death reported `engine_failed`; the preview through the asset protocol, one file at a time; a delete takes the edit list along. Open: the vmtest `record` cell and the field matrix (P6), FFmpeg decode for hardware recordings and AAC after **P4**, a thumbnail strip where the webview cannot play |
...
- [ ] **AC12 — the pointer (P1d).** A local recording on Windows (WGC, and DXGI under `ROOMLERD_CAPTURE=scrap`), macOS
      and Linux X11 shows the mouse pointer where it was, including while it moves over a still screen, and leaves no
      trail; its sidecar's `pointer` says `in_frame` or `drawn`, and a backend that cannot give one (DRM) says `none`.
...
6. ~~**A host at its login screen, for an UNATTENDED recording (P1f)**~~ — **decided 2026-09-26: a login screen is not
   "nobody"** (see "Decided 2026-09-26"). Built: `Refusal::LoginScreen`, refused `login_screen` before the host is asked
   (`docs/recording.md` §6 and §10). It was: a display manager's greeter counted as "nobody", as consent treats it, so a
   remote recording there ran unattended with no banner, while a login screen shows account names and a person may be
   standing at it.
...
| 2026-09-25 19:54Z | production, `hosted-20260925-3f17571` (pre-P3a), the operator's browser | P3a baseline (AC5 "first shown on the current deploy") | The owner asks for `VIEW \| RECORD` on a fresh `/ws` against their own pre-feature Windows device. Answer: `rc:session.created` with `permissions: "VIEW \| RECORD"`, so RECORD was **passed through unstripped**, and no `record_refused` key at all. The socket was closed at once, which tore the session down. Managed role masks: owner 2147483647 (2^31−1, no bit 31), admin 1468006391, moderator 34074513, member 249729, guest 2049, none carrying bit 31. `GET …/recording-activity/{agent_id}` → 404 |
| 2026-09-25 20:06Z | production, `hosted-20260925-d5a7a3d` (P3a live, promote run 36183319196) | P3a post-roll, the pair with the 19:54Z baseline | The same probe as the baseline (owner, `VIEW \| RECORD`, the same pre-feature device). Answer: `permissions: "VIEW"`, `record_refused: "device_not_opted_in"`, so RECORD is now **stripped with its reason**; at 19:54Z it had passed through. Managed masks: owner 4294967295 (bit 31 added by the boot reconcile); admin, moderator, member and guest unchanged, none with bit 31. `GET …/recording-activity/{agent_id}` → 200, `total: 0` (it was 404). Still owed for AC5: the no-role-bit and break-glass cells need a second identity, and the device-side cells need an agent release carrying `recording` |
| 2026-09-26 | the operator, before 0.4.104 | ship step | `recording` taken back out of `full` before any release carried it. It rejoins a release after canary field checks: an EDR-managed corporate laptop first (on Windows the daemon launches a restricted-token child as soon as someone opens the Recordings view, the pattern EDR watches for), then macOS TCC for that child. The kill switch and the macOS `cargo check` stay; the check now names `recording` |
| 2026-09-26 | dev box (Windows, MSVC), release `roomlerd.exe` with the release profile's `CODEGEN_UNITS=1`, features `recording,openh264-encoder,audio`, one worktree and target dir | P5b size | P5a 35,525,120 B → P5b 36,972,544 B: **+1,447,424 B (+1.38 MiB)**. With music limited to MP3, AAC/M4A and WAV: 36,680,704 B (+1.10 MiB), so FLAC and Ogg Vorbis cost 0.28 MiB. ≈ +0.48 MiB in the MSI at the MSZip ratio, above the estimate (0.3) |
| 2026-09-26 | dev box, symphonia 0.5.5 in a scratch crate | P5b | a fuzz-lite over 1,800 malformed music files (random bytes behind each format's magic, hand-made WAV, FLAC and MP3 headers): **one panic**, a WAV declaring 0 Hz (`TimeBase cannot have 0 numerator or denominator`, in the probe); no hang. 1 Hz is accepted, which the resampler would grow 48 000-fold. Both refused `music_unreadable` now, each pinned by a cell shown red first |
| 2026-09-26 | dev box (Windows) + jsdom | P5c | `roomler-desktop` editor unit tests 8/8; `ui` companion suites 46/46 (the editor's 22); roomlerd `recording::edit` + `recording::manager` with the shared fixture and the delete. Nine controls red first: an action merging neighbours, an export not waiting for the save, the Edit button without the engine, a gap accepted, `looped` written by the page, a list naming another recording saved, a silent engine death unreported, a delete leaving the list, `looped` read by roomlerd |
