...
| How do **canary** recorder builds reach hosts? | A separate test build (`full` + `recording`), installed only on the EDR-managed laptop and the Mac, never on the fleet's update channel: through a pre-release channel if the updater has one, otherwise by hand. EDR: open the Recordings view, make a local recording, then a remote one, and watch for alerts or quarantine. Mac: the Screen Recording and Microphone permissions work, and existing grants survive the update. |
