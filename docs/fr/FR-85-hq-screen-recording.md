...
6. **A host at its login screen, for an UNATTENDED recording (P1f)** — today a display manager's greeter is "nobody", as consent treats it, so a remote recording there runs unattended with no banner. A login screen shows account names and a person may be standing at it. The alternative: treat any active graphical session, the greeter's included, as attended, so with no indicator surface the start is refused. The operator decides; raised in review.
