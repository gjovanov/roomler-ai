...
| 1 | `capture::open_default()` at pump start | DXGI/WGC init, synchronous, at the same session-start moment as the encoder open — same class. `open_ms` does **not** catch it: it runs before the loop. | unmeasured |
