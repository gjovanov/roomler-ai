...
| [FR-37](FR-37-e2e-in-cluster.md) | [#928](https://github.com/gjovanov/roomler-ai/issues/928) | The e2e suite runs outside the cluster, so it cannot test media | P1 in progress — **renumbered from FR-36**, which #929 landed first; the ledger caught it in the rebase, before publication. Measured: mars has **no route to the pod network**, so no RTP was ever going to arrive; a service URL is **not a secure context** so `navigator.mediaDevices` is undefined; running the browser in the app pod's own netns fixes both (127.0.0.1 is a secure context AND the SFU announces it). Still red on a fourth blocker: the client stops between `transport_created` and `produce` with healthy devices |
