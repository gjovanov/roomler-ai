...
| [FR-33](FR-33-lan-capture-surfacing.md) | [#905](https://github.com/gjovanov/roomler-ai/issues/905) | A VPN that captures the LAN prefix should say so — surface LAN capture in `roomler status`, `peers --json why` and the RC path pill, so a relay on a LAN pair is attributed in seconds, not days | Designed 2026-08-29 after `neo16 → PC55331` spent five days relay-locked by a Check Point split-prefix capture that every surface reported only as `upgrading` / `relay` / `penalty`. Detect + surface only; P1 status line, P2 `why`, P3 pill |
| [FR-34](FR-34-locked-consent-and-stuck-capture.md) | [#917](https://github.com/gjovanov/roomler-ai/issues/917) | A locked host: a consent prompt you cannot see, and a stream that comes up black | Root-caused 2026-08-29 from pc50045 (locked corp laptop): the FR-27 consent panel is not on the secure desktop, and a DXGI duplication bound during the lock→unlock transition returns empty frames forever (`Transient` never recovers, `capture_pump.rs:562`) — 200k empty / 0 unchanged, black until a reconnect. P1 stuck-capture recovery (gated on frames_delivered==0), P2 first-frame guarantee, P3 consent on the secure desktop |
| [FR-35](FR-35-relay-ceiling-learns-the-pair.md) | [#922](https://github.com/gjovanov/roomler-ai/issues/922) | The constrained ceiling learns the pair — grow the relay cap above the nominal 3 Mbps on delivery evidence (gated by the signals that predicted the stall), remember it per peer so the next session opens there | P0 measured on the REAL `neo16 → PC55331` relay: at a 25.5 Mbps cap the desktop lands as one 680 KB frame at +0.3 s and the path delivers 3–11 Mbps, but chokes at 12.8 Mbps (7.9 s stall) — sustainable ≈ 6–9 Mbps vs the 3 Mbps constant; other corp relays sit at ~2 Mbps. P1 gated growth + P2 per-peer rate memory designed |
