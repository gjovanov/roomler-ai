...
| [FR-19](FR-19-peer-relays.md) | [#805](https://github.com/gjovanov/roomler-ai/issues/805) | Peer relays — tenant-owned UDP relay nodes between direct and DERP | proposed — independently reviewed on 3 lenses pre-publication; carrier is a third `RelayKind` (not a new tier), bind is key-authenticated, **E2E-3 RUN: port settled at 3478** (CORPLAP-3 reaches 3478 and nothing else); P0–P2 and P3a SHIPPED (step log on #805: bind-only responder → forwarding relay field-verified on scw-m2-asahi → models/wire); P3b (#893, DAO/routes/audit/fail-closed loaders) merging; **P3c the mint** (#899) merging; **P4a** the client (bind handshake + `OrgRelayConn`, loopback-proven) + **P4b** runtime/agent wiring (`RelayKind::Org`, the strategy branch, `relay_serve`/`relay_revoke` handling, probe report, join flags) in PR behind `overlay_org_relay` (default off); P4c the first field mint next |
