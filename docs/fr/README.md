...
| [FR-36](FR-36-e2e-in-cluster.md) | [#TBD](https://github.com/gjovanov/roomler-ai/issues) | The e2e suite runs outside the cluster, so it cannot test media | proposed — measured 2026-08-29: the e2e pod has a CNI podIP and mars has **no route** to the pod network (`10.244.3.196 via 94.130.141.65 dev eth0`), so no RTP was ever going to arrive. ⚠️ NOT a firewall/DNAT problem, as assumed — the browser is in the wrong network. Move the browser in-cluster as a Job; that also removes the port-forward supervisor and the cookie-origin override that the WS-403 outage needed |
