...
| [FR-36](FR-36-wayland-capture.md) | [#929](https://github.com/gjovanov/roomler-ai/issues/929) | Wayland capture — the desktops we cannot see at all | design — a Wayland session produces a **black stream today**, and on Apple Silicon that is every accelerated desktop. Also the only remaining lever on the ~27 fps ceiling after **FR-29 measured X11 partial readback dead** (a window drag reports ONE full-screen damage rect). ⚠️ The blocking problem is not pixels: the portal is a **per-user session service** while the agent runs as root, and the grant is **interactive** — so P0 is a process-model decision, and the unattended half must land WITH FR-27 consent, not around it |
