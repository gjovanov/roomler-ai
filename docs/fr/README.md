...
| FR-19 | [#805](https://github.com/gjovanov/roomler-ai/issues/805) | Peer relays — tenant-owned UDP relay nodes between direct and DERP | claimed by #805; **spec lands with its own PR** — row added by the FR-20 session, which found the number taken |
| [FR-20](FR-20-per-tenant-relay-cost-metering.md) | [#807](https://github.com/gjovanov/roomler-ai/issues/807) | Per-tenant relay cost metering — attribute what relay traffic actually costs, per org | design — the shipped stats subsystem measures fleet health (`stats_relay` is keyed `{region, ts}`), never consumption; ships no quota by design |
