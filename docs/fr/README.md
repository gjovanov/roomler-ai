...
| [FR-34](FR-34-locked-consent-and-stuck-capture.md) | [#TBD](https://github.com/gjovanov/roomler-ai/issues) | A locked host: a consent prompt you cannot see, and a stream that comes up black | Root-caused 2026-08-29 from pc50045 (locked corp laptop): the FR-27 consent panel is not on the secure desktop, and a DXGI duplication bound during the lock→unlock transition returns empty frames forever (`Transient` never recovers, `capture_pump.rs:562`) — 200k empty / 0 unchanged, black until a reconnect. P1 stuck-capture recovery (gated on frames_delivered==0), P2 first-frame guarantee, P3 consent on the secure desktop |
