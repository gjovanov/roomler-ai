...
| [FR-19](FR-19-peer-relays.md) | [#805](https://github.com/gjovanov/roomler-ai/issues/805) | Peer relays — tenant-owned UDP relay nodes between direct and DERP | proposed — independently reviewed on 3 lenses pre-publication; carrier is a third `RelayKind` (not a new tier), bind is key-authenticated, **E2E-3 RUN: port settled at 3478** (CORPLAP-3 reaches 3478 and nothing else); P0 wire forward-compat SHIPPED (#811). P1 next |
...
| [FR-27](FR-27-consent-surfaces-and-desktop-companion.md) | [#854](https://github.com/gjovanov/roomler-ai/issues/854) | Host consent — every mode, every OS, and a desktop companion that works | shipped (#858), **Windows native panel field-verified on 0.4.16**; the field test found 3 more defects (#877), one of which froze every pre-0.4.16 Linux agent. Open: `prompt_owner`, `no_prompt_surface`, exec/ssh prompts, macOS + X11 panels |
