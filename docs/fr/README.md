...
| [FR-21](FR-21-retire-obsolete-binary-names.md) | [#809](https://github.com/gjovanov/roomler-ai/issues/809) | Retire the obsolete `roomler-agent` / `roomler-agent-tray` names — code, docs, file/folder names, all three OSes | **closed 2026-08-29** — all 11 acceptance criteria met; 1 849 → **0** unclassified, 743 anchored; CI blocks on `name-audit.sh --check --strict`; field-verified on Linux, Windows and macOS, because the hard part is the freeze list (7 dual-read anchors, incl. the TCC-keyed macOS bundle) |
...
| [FR-32](FR-32-plan-limit-enforcement.md) | [#898](https://github.com/gjovanov/roomler-ai/issues/898) | Eleven of fourteen plan limits are advertised and enforced nowhere | design — P1 of the pricing arc (enforce → meter → re-price → keys, never inverted). Only `max_devices` (x2) and `max_tunnel_clients` are enforced; the other 11 are published by `/api/stripe/plans` and read back by nothing. Two findings beyond the prior evaluation: `TenantSettings.{max_members,file_upload_limit}` shadow two of them with **zero readers anywhere**, and the `count_*` DAOs exist only for the two limits already enforced — the counting, not the `if`, is the work. Ships in `Warn` (the `acl_mode` precedent) because flipping 11 gates on a live fleet locks out tenants we put over the line | **Renumbered from FR-31**: #897 (opening-keyframe budget) claimed it with the lower issue id, and the lower id keeps the number. FR-32 was verified free rather than vacated (`git log -S "FR-32"` on the ledger returns nothing, and no issue was ever titled FR-32) — a vacated number must never be reused 
| [FR-36](FR-36-wayland-capture.md) | [#929](https://github.com/gjovanov/roomler-ai/issues/929) | Wayland capture, and unattended access | design — **P0 DECIDED: DRM/KMS first, portal demoted to fallback.** A Wayland desktop is **uncapturable today**, and on Apple Silicon that is every accelerated desktop. ⚠️ The portal is structurally an ATTENDED api: a LOCKED session makes mutter refuse to create *or restore* a screencast, and the failed attempt CONSUMES the restore token — so `persist_mode` cannot deliver unattended. Prior art (RustDesk) capture below the compositor via `drmModeGetFB2`; **`libdrmtap` is MIT** and usable, **RustDesk itself is AGPL-3.0 — do not copy its source** (FR-24). Host prereqs all measured present; ⚠️ Asahi detiling is the big unknown |
| [FR-37](FR-37-e2e-in-cluster.md) | [#928](https://github.com/gjovanov/roomler-ai/issues/928) | The e2e suite runs outside the cluster, so it cannot test media | **P1 done — conference specs PASS in-cluster** (`conference` 4/4, `conference-multi` 5/5, was 0/9). Three findings: mars has **no route to the pod network**; a service URL is **not a secure context** so `navigator.mediaDevices` is undefined; running the browser in the app pod's netns fixes both. ⚠️ The last blocker was a PRODUCT bug — a blank `turn.url` advertised an ICE server with an empty URL and **no one could join a call in any browser** (#940). The expected-failures file blamed RTC ports for months. P2 (wire the nightly to a Job) and P3 (re-triage the baseline) open |
