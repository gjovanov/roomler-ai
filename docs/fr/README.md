...
| [FR-30](FR-30-camera-state-signalling.md) | [#884](https://github.com/gjovanov/roomler-ai/issues/884) | Camera/mic state is never signalled — peers cannot see who turned their camera off | proposed — found while field-verifying FR-25: `toggleVideo` pauses the producer LOCALLY and nothing is relayed, so a receiver measures `enabled:true/live/unmuted` after the sender's camera goes off. Mirrors the existing `media:producer_close`/`producer_closed` pair; also stops a camera-off participant shipping black frames to everyone |
