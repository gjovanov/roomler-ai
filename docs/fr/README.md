...
| [FR-17](FR-17-partial-reliability-video.md) | [#799](https://github.com/gjovanov/roomler-ai/issues/799) | Video rides a reliable+ordered DataChannel — HOL blocking costs SECONDS on a relay | **stage A shipped** (#820) — per-chunk framing so a receiver can see a gap at all, negotiated via `AgentCaps.video`; inert until stage B flips ordering. `send_wait_max_ms` measured at 10 263 ms on a healthy agent; child of FR-1 |
...
| [FR-23](FR-23-company-identity-and-domain-consolidation.md) | [#827](https://github.com/gjovanov/roomler-ai/issues/827) | Company identity on the site, and one product on both domains | in progress — phases 0–3 **shipped + field-verified** on `v20260828-de59383da2fe` (`grep imprint` on the live bundle 0→1; the entity, UIC, office, corrected phone and both domains all present in the shipped chunk). #828 + #830 (the imprint I shipped cited the EU ODR platform, repealed 2025-07-20). Open: send Apple the correction; `roomler-old` teardown (inventoried, awaiting go-ahead); `roomler.live` — **blocked on a decision**, since `ROOMLER__APP__CORS_ORIGINS` cannot be set from a configmap at all (no `list_separator` ⇒ boot crash) and a 301 now looks better than serving the app. Unblocks the Apple half of FR-7 |
| [FR-24](FR-24-licensing-split.md) | [#838](https://github.com/gjovanov/roomler-ai/issues/838) | Licensing split — AGPL-3.0 control plane, MPL-2.0 everything the customer runs | P1–P4a shipped; AGPL covers ~37% of the tree and 0% of the cascades (both are SHARED/CLIENT) — kept for the sellable exception, and `LICENSING.md` says so; CLA draft + LGPL §6 relink gap open |
