...
- ~~**`av1_vulkan` on NVIDIA emits a stream Chrome cannot decode**~~ Attributed and
  fixed the same day (P3 field log): FFmpeg 9.0.1's `av1_vulkan` writes no
  temporal-delimiter OBU at the start of a temporal unit, which every other AV1
  encoder we ship does and Chrome's decoders require; `FfmpegEncoder::drain_packets`
  prepends it for any AV1 backend that omits it (0.4.91). What stays open is only the
  session re-test on that build, and — for RADV on VCN 4/5, where `av1_vulkan` would be
  the ONLY AV1 cell — whether the same repair is all that driver needs; no fleet host
  has that silicon yet. A packet-level repair was chosen over a denylist entry because
  the cascade already prefers NVENC/QSV/AMF wherever they exist, so the cell is only
  reachable on hosts with no other AV1 encoder, exactly the hosts a deny would strand.
