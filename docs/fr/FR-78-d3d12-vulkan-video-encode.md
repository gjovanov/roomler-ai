...
| 2026-09-08 | release + the fleet roll, and the containment through the server record | the denylist entry | `agent-v0.4.92` (#1520 → bump #1523 → `16f6be742`; release run 34255622430, 28 assets — the macOS `.pkg` job spent 45 min in Apple's notarisation queue, unrelated to this change). **All seven hosts back on 0.4.92 in 2 minutes**, the fastest roll of the arc. Server records: **the dev box advertises 12 cells — `hevc/d3d12`, `hevc/vulkan`, `h264/d3d12`, `h264/vulkan` all present, `av1/vulkan` gone**, the NVENC cells still carrying `yuv444`; CORPLAP-3 keeps its seven with `hevc/d3d12`; jupiter is back to six (its test denylist cleared, VAAPI + Vulkan); zeus, mars, the WSL sibling and the MacBook unchanged. The containment is confirmed where a controller reads it, not only in the probe |
