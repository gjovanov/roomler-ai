...
| P1 | model + reaper | `ephemeral` + `ephemeral_ttl_secs` on `Agent`/`TunnelClient`; `remove_agent` factored out of `delete_agent` (F3); a cluster-singleton `reap_ephemeral` loop with its own query (F2); hard delete (F4) | `rc.ephemeral_reaper_enabled`, default **false** ⇒ zero queries, zero deletes |
| P2 | ephemeral enrollment keys | `POST …/agent/enroll-key` minting a reusable, capped, revocable, audited key (§4); the flag rides the token; an `enrollment_keys` collection + `DELETE` to revoke; per-use audit | per-org `ephemeral_keys_enabled`, default **off** ⇒ the route mints nothing |
