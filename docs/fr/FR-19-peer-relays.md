...
| **CORPLAP-2** | **NO MAPPING** | BLOCKED | untyped | ❌ UDP is dead; stays on the DERP floor |
...
1. **A relay on a high UDP port would be unreachable by its own target audience.** The
   reference documents `tailscale set --relay-server-port=40000`. On `CORPLAP-3` — corp
...
2. **`CORPLAP-3` is the proof the feature is worth building.** Its NAT is *symmetric* —
...
3. **`CORPLAP-2` is the proof DERP must stay.** `stun/udp: NO MAPPING` = no UDP path at all.
...
- **The floor is unconditional.** DERP registration is never torn down because an org relay
  came up. `CORPLAP-2` must be measurably unaffected by this entire feature.
...
- [ ] `CORPLAP-2` (`stun/udp: NO MAPPING`) stays on `relay:derp/tcp` throughout, with no added
...
- [ ] `CORPLAP-3` — symmetric NAT, `relay band/udp: BLOCKED`, currently `relay:derp/tcp` —
...
**E2E-1 — relay selection.** All three relays offered to `CORPLAP-3`; assert it picks the
...
**E2E-2 — the floor holds.** With all three relays live, assert `CORPLAP-2` is still
...
{CORPLAP-3, CORPLAP-2, CORPLAP-1}, recorded from `netcheck`. This table confirms or refutes
...
⚠️ It **measures clock skew per run** (`run-lab.sh:44-64`) because CORPLAP-1 sat 21.4 s behind
