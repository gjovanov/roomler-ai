...
- [ ] Same pair, same day, before/after: RTT ✓, overlay throughput ✓ (F2 = **3.18×**), and
      **SSH-over-overlay throughput** — *re-scoped from "RC `send_wait_max_ms`" (2026-08-30):
      ...
      carrier (client → overlay TUN → WG → org/DERP/direct → target).* SSH `CORPLAP-3`→mars
      measured **DERP 58.9 / direct 44.4 Mbps** (60 MB `dd`, carrier-confirmed); org leg deferred
      — the host's org climb was re-request-backoff-blocked this session, and org's advantage on
      this exact pair is established via the 3.18× UDP result.
      [#805 comment](https://github.com/gjovanov/roomler-ai/issues/805#issuecomment-5480692218).
...
| F3 | **SSH-over-overlay throughput** (re-scoped from RC `send_wait_max`, 2026-08-30 — RC doesn't ride the overlay) | DERP **58.9** / direct 44.4 Mbps; org *pending* | org > DERP (per F2's 3.18× carrier delta) |
