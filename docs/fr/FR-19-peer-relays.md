...
<!-- RETIRED-NAME-ANCHOR: the row below quotes the singleton-lock message VERBATIM as the operator saw it on the host during the kill test. The daemon says "Another roomlerd is already running" today (main.rs); this is the record of what that host actually printed, and rewriting a field log to match current code would falsify it. Same treatment as the historical appendices. docs/fr/FR-21 -->
