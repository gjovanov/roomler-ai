...
**Issue:** [#1041](https://github.com/gjovanov/roomler-ai/issues/1041) · **Status:** P1 → P3c-i shipped and field-verified — **frames arrive from a Wayland desktop with no DRM capture**. P3c-ii (frames to the daemon) next · **Owner:** agent / capture
...
| **P3c-ii** | Get frames to the daemon — buffer fds over `SCM_RIGHTS` once at negotiation, then a ready-message per frame — surfacing through the existing `ScreenCapture` trait as a **sixth backend**. | same |
