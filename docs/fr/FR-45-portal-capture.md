...
**Issue:** [#1041](https://github.com/gjovanov/roomler-ai/issues/1041) · **Status:** P1 + P2a + P2b + P3a + P3b shipped and field-verified — a format is negotiated end to end. P3c (frames) next · **Owner:** agent / capture
...
| **P3c** | Buffer delivery: `pw_stream_dequeue_buffer`, buffer fds over `SCM_RIGHTS` once at negotiation then a ready-message per frame, surfacing through the existing `ScreenCapture` trait as a **sixth backend**. | same |
