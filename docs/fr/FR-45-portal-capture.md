...
- [x] **P4: input lands.** Field-verified on `scw-m2-asahi` (GNOME Wayland,
      ...
      evdev inversion would have been backwards. ⏳ Typed text and click were
      NOT exercised (no safe target on a desktop with live apps); the pointer
      and wheel paths carry the same `plan`→`execute` machinery
