...
| 2 | **First-frame guarantee** | P1 already prevents a *permanent* black on a static screen (its GDI backstop delivers a frame) but only after the 2 s stuck window. P2 shortens the wait: when DXGI has delivered no frame within `FIRST_FRAME_GDI_AFTER` (300 ms), the SystemContext worker primes **one** GDI BitBlt frame (always the current screen) so the controller sees the desktop ~instantly on connect — **without** swapping the backend or setting `delivered_since_build` (DXGI takes over at full quality the moment it delivers, and a truly-stuck DXGI is still caught by the 2 s `capture_is_stuck` path). One-shot per session. | `ROOMLERD_STUCK_CAPTURE_RECOVERY=0` (shared with P1) |
