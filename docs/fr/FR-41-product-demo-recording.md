...
| 2026-08-30 | **P0: the sibling joined `demo` as a SECONDARY org — GROX intact** | `roomlerd enroll --label demo` against a token for a new org on the SAME server **appends** rather than replaces (the flag's documented behaviour, confirmed live): `wsl-demo` is online in `demo` AND `NEO16-WSL` is still online in GROX. Option B therefore cost nothing — the irreversible-removal risk this FR was staged around does not apply when the target is a second org on one server |
