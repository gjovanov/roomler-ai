...
**Status:** in progress — P1 · P2a–c · P3a–c shipped: an outsider can PROVE the password to a device through the server (single-pod), but **no session follows yet** (P4) ·
...
| P3d | **Cross-pod.** An outsider has NO tenant, so tenant affinity cannot put their socket on the pod holding the device, and `/ws` refuses a `tid` they are not a member of — in a 2-replica deployment roughly half of them land on a pod that cannot reach it. The frames carry the connect code (not an agent id) precisely so the pod they land on can re-resolve and forward them over the PR-2 rc relay to the owner pod. Until then that case answers `unavailable` and the audit says why. | same | not started |
