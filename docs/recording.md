...
- ⚠️ **A drop is read from the channel's STATE, not only from its callback.**
  A close from the FAR end always reaches `Closed` and fires `on_close`. A
  channel closed from the device's own side is set `Closing`, and whether it
  then reaches `Closed` and fires `on_close` is a race inside webrtc-rs's read
  ...
  the device's session watchdog closes the peer itself. So `session_gone`
  runs once, from whichever comes first: `on_close`, or a look at the channel
  every 500 ms (`channel_gone`, which is `Closing` or `Closed`). Three places
  use that same test: the follower, the download pump, and the controller's
  next session. The next session detaches a holder of its own whose channel is
  gone before it answers the status. Found by the Linux lane, where the cells
  closed the controller's peer first. On every Windows run its reset reached
  the device first; on Linux it did not. The recording then ran on, never
  detached, and another controller's session never saw it end. The cells now
  drop the way a network does, with the device's peer closing first.
- **One speaker at a time.** Every follower takes a number (`FOLLOWER`); one
  whose recording moved on stops quietly, so an ending is reported once, on
  the session that held it last. A stop a session asked for itself (the
  controller's Stop, the owner's OFF) is not detached if the session drops
  while the file finalizes: that session's follower still reports it.
