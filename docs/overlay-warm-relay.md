...
> after the next VPN-on window on CORPLAP-1: (1) W5 phase-2 — does the
...
Check Point-class corp VPNs (CORPLAP-1, field-proven 2026-08-14) block
...
2. **Stage 1 — establish + keep alive only** (no pairing use): field-gate
   on CORPLAP-1 = the allocation survives a VPN connect (refresh keeps
...
3. **Stage 2 — anchor prefers the warm allocation** for new pairs when
   ...
   CORPLAP-1 pairs read `relay:turn/udp` (not `tcp`), and the disco
