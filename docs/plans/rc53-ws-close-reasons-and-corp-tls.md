...
Two failure modes burned operator hours in the field this week (CORPLAP-2 +
...
2. **Corporate networks with SSL-inspection middleboxes.** rc.31/rc.32
   fixed the TURN/ICE path for CORP1-style hosts (`Symantec Enterprise
   ...
   `wss://roomler.ai/ws` (today it doesn't — CORP1 tunnels direct via
...
**New in v1** (driven by today's CORPLAP-2 finding, was missing from v0):
