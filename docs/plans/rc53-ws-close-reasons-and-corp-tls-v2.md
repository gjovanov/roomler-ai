...
| 12 | **Phase 7 ADDED** (~15 LOC stderr warning in `cmd_enroll` at `agents/roomler-agent/src/main.rs:792-860`) for the same-session `%APPDATA% ↔ %PROGRAMDATA%` asymmetry — CORPLAP-2's current pain. Cheaper + higher-value than Phase 6. | #12 |
...
Two failure modes burned operator hours this week (CORPLAP-2 + the rc.48-era SystemContext loop):
