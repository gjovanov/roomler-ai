...
| Component | Mechanism |
|---|---|
| `roomlerd` (daemon hosts) | Self-updater polls `/api/agent/latest-release` (24 h timer + startup cooldown), verifies SHA-256, hands off to MSI / `dpkg` / `installer` and restarts. Admins can force it fleet-wide or per device (`POST …/agent/update`); ⚠️ a device with `auto_update = false` ignores the push — the server still answers `delivered: true`, and the device does not say it ignored it (#1689). Crash-looping updates roll back to the last known-good version |
