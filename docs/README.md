...
| [remote-apps.md](remote-apps.md) | Remote Apps (FR-56): the three verbs on the control data channel (`rc:apps.list` · `focus` · `launch`) and their replies; how the Linux backend finds a desktop from a root daemon that has none — the daemon's own Xvfb first, then whoever is at the screen, `DISPLAY` **and** `XAUTHORITY`, privilege-dropped to the session's owner; the **tier-by-tier honesty table** — what each tier advertises in the hello, what its reply says and what the panel shows (X11-only listing on Wayland, GNOME's refused enumeration, the missing helpers, nobody logged in, no Xwayland, disabled, macOS); `coverage` and `unavailable`, and why the hello is a boot-time snapshot the live reply overrides; the tmux session model and the launch allowlist; per-window capture (P4); the field table; configuration and `roomlerd apps-probe`; a code map |
