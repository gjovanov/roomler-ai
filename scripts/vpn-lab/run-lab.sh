#!/bin/bash
# run-lab.sh — one command = one VPN-cycle experiment against pc50045.
#
...
psexec() { # run an encoded PS command on pc50045, CLIXML noise stripped
  local enc; enc=$(printf '%s' "$1" | iconv -f UTF-8 -t UTF-16LE | base64 -w0)
  "$EXEC" exec PC50045 --timeout "${2:-60000}" -- "powershell -NoProfile -EncodedCommand $enc" 2>&1 |
    grep -vE "CLIXML|^<Objs"
}
