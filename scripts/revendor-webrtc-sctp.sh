...
#   scripts/revendor-webrtc-sctp.sh --check [version]   verify tree == upstream+patch
#                                                      (CI drift gate; no writes)
#   scripts/revendor-webrtc-sctp.sh --regen [version]   regenerate the .patch from
#                                                      the CURRENT vendored tree
#                                                      (after editing the fork)
#
# Default version: 0.11.0 (the vendored release). After a rebuild, run
...
VENDORED="$ROOT/crates/vendored/webrtc-sctp"
PATCH="$ROOT/crates/vendored/webrtc-sctp.patch"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
curl -sfL "https://static.crates.io/crates/webrtc-sctp/webrtc-sctp-$VER.crate" |
...
check | revendor)
    ...
    if [ "$MODE" = "check" ]; then
        if diff -r --strip-trailing-cr "$TMP/rebuilt" "$VENDORED" >"$TMP/drift.txt" 2>&1; then
            echo "OK: vendored webrtc-sctp == upstream $VER + $(basename "$PATCH")"
        else
            echo "DRIFT: vendored webrtc-sctp != upstream $VER + patch:" >&2
            head -40 "$TMP/drift.txt" >&2
            echo "(edit the fork, then run: scripts/revendor-webrtc-sctp.sh --regen)" >&2
            exit 1
        fi
    else
        rm -rf "$VENDORED"
        mv "$TMP/rebuilt" "$VENDORED"
        echo "revendored webrtc-sctp $VER + patch into crates/vendored/webrtc-sctp"
