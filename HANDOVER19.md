...
> Continuation of HANDOVER18. This session shipped rc.22 with always-PROGRAMDATA staging on Windows (Option B from HANDOVER18 Bug 1) and wrote the M3 elevated/user-app input plan (HANDOVER18 Bug 2). CORPLAP-1 verification pending.
...
## Verification needed on CORPLAP-1
...
- **If Bug 1 closed by rc.22**: move to Bug 2. Bisect rc.7 → rc.21 on CORPLAP-1 per `docs/remote-control-m3-elevated-switching.md` §"Required pre-coding evidence". Implement Change A first (~30 LOC in `lock_state.rs`); ship rc.23.
...
- `#166 [pending] rc.19 P8: manual smoke on CORPLAP-1` — superseded by rc.22 verification above (covers the same kill-position scenarios + the new ESET hypothesis).

## How to pick up next session

1. Read this file first.
2. Read `docs/remote-control-m3-elevated-switching.md` for the Bug 2 plan.
3. User should have run the rc.22 verification on CORPLAP-1 by then; ask for the result.
