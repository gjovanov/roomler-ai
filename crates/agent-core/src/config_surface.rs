...
const KEYS: &[(&str, &str, &str)] = &[
    ...
    (
        "overlay_v6_win",
        "tribool",
        "Pin the overlay adapter's IPv6 interface metric and assert the ULA /96 at the defended route metric (1) instead of the stock connected-route 256, so IPv6 wins a contested prefix the way IPv4 already does (Windows). Built-in default: off. Without it the adapter keeps Windows' AUTOMATIC v6 metric and the ULA sits at 256: measured against a corp VPN that is 261 for us vs 26 for the VPN, lost outright, which is why the route guard evicts the VPN's mirrored /96 about 20 times a minute forever on a host whose IPv4 is quiet. Not the metric-0 variant a VPN route monitor deletes — it uses the same metric 1 IPv4 runs fleet-wide.",
    ),
    (
        "local_turn",
        "tribool",
...
fn current_value(cfg: &AgentConfig, key: &str) -> Option<String> {
    match key {
        ...
        "overlay_v6_win" => cfg.overlay_v6_win.map(fmt_bool),
...
pub fn apply(cfg: &mut AgentConfig, key: &str, value: Option<&str>) -> Result<(), String> {
    match key {
        ...
        "overlay_v6_win" => cfg.overlay_v6_win = parse_tribool(value)?,
