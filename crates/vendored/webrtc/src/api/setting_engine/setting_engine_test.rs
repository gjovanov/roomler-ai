...
// Roomler patch — locks the SCTP rwnd setter/getter so an upstream
// Cargo bump can't silently un-do the patch. The default of 0 must
// preserve the upstream 1 MiB fallback; a non-zero value must round-
// trip exactly. Consumed by RTCSctpTransport::start at
// src/sctp_transport/mod.rs:159.
#[test]
fn test_sctp_max_receive_buffer_size_roundtrip() -> Result<()> {
    let mut s = SettingEngine::default();
    assert_eq!(
        s.get_sctp_max_receive_buffer_size(),
        0,
        "default must be 0 so upstream INITIAL_RECV_BUF_SIZE applies"
    );

    s.set_sctp_max_receive_buffer_size(8 * 1024 * 1024);
    assert_eq!(s.get_sctp_max_receive_buffer_size(), 8 * 1024 * 1024);

    s.set_sctp_max_receive_buffer_size(0);
    assert_eq!(
        s.get_sctp_max_receive_buffer_size(),
        0,
        "reset to 0 must restore upstream default behaviour"
    );
