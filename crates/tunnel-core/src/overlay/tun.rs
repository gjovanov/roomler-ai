...
pub trait TunIo: Send + Sync {
    ...
    /// enough on such a host (field bug 2026-06-10: CORPLAP-1's pings to peers
    ...
    /// full-tunnel VPN installs a `/32` for it too. Field (CORPLAP-1, Check Point
    /// Endpoint, 2026-07-31):
    ///
    /// ```text
    ...
    /// CORP GATEWAY (field 2026-08-08, CORPLAP-1: `Antwort von 10.16.6.34:
...
mod system {
    ...
    mod winroute {
        ...
        pub fn ensure(luid: u64, dest: IpAddr, plen: u8, metric: u32) -> std::io::Result<()> {
            ...
            if rc != NO_ERROR {
                ...
                // something outside this process deleted a row we wrote (CORPLAP-3
    ...
    /// (CORPLAP-1/CORPLAP-2) already wins with eviction at metric 1. A default
    ...
    impl TunIo for SystemTun {
        ...
        async fn defend_self_route(&self, self_ip: Ipv4Addr) {
            #[cfg(target_os = "windows")]
            {
                ...
                // rc.281 — the v6 twin. IPv6 survived the CORPLAP-1 hijack only
        ...
        async fn verify_peer_path_ownership(&self, peers: &[Ipv4Addr]) {
            #[cfg(windows)]
            {
                ...
                // (CORPLAP-1's Check Point NIC and CORPLAP-2's both sit at 0),
