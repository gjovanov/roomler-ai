...
//! LAN IP — field host CORPLAP-1 routes the internet via corporate Ethernet but
...
/// picked the WRONG interface on a multi-homed host: field host CORPLAP-1 routes
    ...
    if let Ok(addrs) = if_addrs::get_if_addrs() {
        for a in addrs {
                ...
                && !out.iter().any(|(existing, _)| *existing == ip)
            {
                ...
                // interfaces (see `lan_iface_denied`). Field: CORPLAP-1
...
/// do. Field 2026-08-15 CORPLAP-2: srflx `cone via 5.9.157.221:3478` from the
...
/// full-tunnel corporate VPN's captured default route. Confirmed on CORP1
...
/// `overlay_derp_floor`. **Default-ON since rc.400** (neo16+CORPLAP-3 soak
/// 08-17: floor pairs carried straight through CORPLAP-3's latch re-earn windows
...
/// the daemon's lifetime (field 2026-08-14: CORPLAP-1 on the corp VPN stayed
...
mod tests {
    ...
    /// interface inventory: CORPLAP-1's WSL vEthernet + Check Point adapter
    ...
    fn lan_iface_denied_matrix() {
        ...
        let cases: &[(&str, &str, bool)] = &[
            // CORPLAP-1's poison trio (2026-07-30):
            (
    ...
    fn same_subnet_24_matches_lan_pairs() {
        ...
        assert!(same_subnet_24(a, b), "CORPLAP-1 + NEO16 are same /24");
    ...
    fn multi_homed_host_matches_on_the_right_interface() {
        // rc.132 regression guard: CORPLAP-1's bug. The node is multi-homed —
