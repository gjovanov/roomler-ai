...
///
/// `UNROUTED`: no registration for the source pubkey — a peer is relaying to us
/// while we hold it on a different carrier. That IS the demote-follow's input,
/// so on a healthy mesh it ticks briefly around a transition and then stops;
/// climbing steadily means follows are not converging the pair.
///
/// `BACKPRESSURE`: a LIVE carrier whose inbound queue is full — the consumer
/// stopped draining. A different fault, deliberately counted apart.
///
