...
//! FR-69 P1 — the server core, split out of `AppState`.
//!
...
//! `AppState` keeps `core` as its first field and **derefs to it**, so every
//! `state.settings` / `state.users` / `state.db` in the tree reads exactly as
//! before — the split changes ownership, not call sites.
//!
//! The handler-side seam is `State<Core>`: a handler that needs only the
//! core takes it instead of `AppState`, which is what the module crates will
//! take from P2 on (`impl FromRef<AppState> for Core` is what makes the
//! extractor work while the router's state is still `AppState`).
//!
//! This type moves into `roomler-core` in P1b unchanged; it lives here first
//! so the split and the move are two reviewable diffs rather than one.
...
/// `State<Core>` in a handler while the router's state is still `AppState`.
