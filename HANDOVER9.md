...
7. ~~Agent auto-start on boot / login~~ — **SHIPPED 0.1.36**.
   `roomler-agent service install|uninstall|status` CLI hooks a
   Scheduled Task (Windows, ONLOGON + LIMITED), a systemd user unit
   (Linux), or a LaunchAgent plist (macOS). Idempotent; shells out
   to the OS tool (`schtasks` / `systemctl --user` / `launchctl`)
   rather than touching registries directly. `packaging/windows/README.txt`
   updated to recommend `service install` over the raw `schtasks`
   snippet.
8. ~~Agent version checker + auto-updater~~ — **SHIPPED 0.1.36**.
   `updater` module polls `https://api.github.com/repos/gjovanov/roomler-ai/releases/latest`
   every 6 h (+ on startup), compares tag vs `CARGO_PKG_VERSION`,
   downloads the platform-appropriate installer, spawns it detached
   (`msiexec /qn` / `pkexec apt-get install` / `installer -pkg`),
   and exits so the installer can overwrite the binary. Service hook
   relaunches. Disable via `ROOMLER_AGENT_AUTO_UPDATE=0`. Manual
   check via `roomler-agent self-update --check-only`.
