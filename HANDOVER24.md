...
- **CORPLAP-2 field investigation** — diagnosed end-to-end. The hours of WS-fail
  ...
  access.
- **Open user-facing issue at session end:** CORPLAP-2 operator is hitting the
...
| 8 | **Missing the same-session `%APPDATA% / %PROGRAMDATA%` asymmetry.** CORPLAP-2's current pain. Added as Phase 7 (~15 LOC stderr warn in `cmd_enroll`). | **MED-add** |
...
- CORPLAP-2 specifically: operator needs to either start the SCM service (to use the machine-global config they enrolled) OR re-enrol without `--machine-global` (so user-context `run` reads %APPDATA%). Doc has the matrix at section 6.
