...
fn print_status(s: &NodeStatus) {
    ...
    // non-zero: on a healthy node both are 0 and a permanent zero line is
    // noise, but after a network transition `unrouted` is the first thing to
    // read — it means a peer is relaying to us that we have not followed onto
    // DERP, which is exactly the 2026-08-25 case that could not be diagnosed
    // because these counters existed and were readable nowhere.
