...
/// loop never makes forward progress (field repro: CORPLAP-1
...
///   (the 2026-05-10 CORPLAP-1 field repro). `/qb!` shows a minimal
...
pub fn spawn_installer_inner(installer_path: &std::path::Path) -> Result<u32> {
    ...
    // NEO16; see BLOCKER B6 in the rc.27/rc.28 master plan.
    #[cfg(target_os = "windows")]
    {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        match flavour {
            ...
            WindowsInstallFlavour::PerMachine => {
                ...
                // %ProgramFiles%. Field bug 2026-05-10 CORPLAP-1: plain
...
mod tests {
    ...
    fn classify_wizard_exe_locations_returns_peruser_not_permachine() {
        ...
        // Field repro 2026-05-15 on NEO16: wizard ran from
    ...
    fn msiexec_argv_per_machine_uses_qb_bang() {
        ...
        // prompt and silently fail (field bug CORPLAP-1 2026-05-10).
