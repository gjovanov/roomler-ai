...
async fn media_pump_vp9_444_dc(
    ...
    // metronome (field NEO16→CORPLAP-2 2026-07-27: text pulsing blur→crystal
...
impl FfmpegDcCodec {
    ...
    /// prefix (field: every CORPLAP-3 rebuild burned ~100-300 ms failing
...
async fn media_pump_ffmpeg_dc(
    ...
    // NEO16→CORPLAP-2 2026-07-27: blur→crystal text pulse on all codecs,
    ...
    // under window movement (field NEO16: 46 fps with periodic
    ...
    // stalled capture+encode → the periodic freeze (field NEO16).
    ...
    // the dead cascade prefix (CORPLAP-3 field: an av1_nvenc tiered-open attempt
    ...
    loop {
        ...
        // amplifying the stall. Field NEO16 (RTX 5090, 2560×1600):
        ...
        let frame: std::sync::Arc<crate::capture::Frame> = match next {
            ...
            Ok(None) => {
                    ...
                    && let Some(ref f) = last_good_frame
                {
                    ...
                    // NEO16 2560×1600 SystemContext: cap 7↔117ms,
        ...
        // which let light motion slip through — field CORPLAP-3 (GDI + av1):
        ...
        let Some(dc) = dc_arc else {
            ...
            // (field: NEO16 HEVC). media_pump_vp9_444_dc already
...
///   blur↔crystallize AIMD sawtooth (field NEO16→CORPLAP-1 2026-07-16), so it
