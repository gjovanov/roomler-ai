...
mod quality {
    ...
    /// rc.36 (CORPLAP-1 / CORPLAP-3 field test 2026-05-17) after the
    ...
    loop {
        ...
        if encoder_dims != Some((frame.width, frame.height)) {
            ...
            if !auto_downscale_evaluated {
                ...
                // (CORPLAP-1 field bug 2026-05-17).
                //
...
async fn media_pump_vp9_444_dc(
    ...
    //
    // rc.43 — RETUNED for VBR-mode regression. Field log CORPLAP-1
    ...
    // rc.45 — dynamic cpu-used boost during motion. CORPLAP-1 field
        ...
        Box::pin(async move {
            ...
            match t {
                ...
                "rc:logs-fetch" => {
                    ...
                    // — field repro CORPLAP-1 2026-05-13 showed
...
fn attach_cursor_handler(dc: Arc<RTCDataChannel>, session_id: bson::oid::ObjectId) {
    ...
    tokio::spawn(async move {
        ...
        // rc.38 — bumped 33 ms (30 Hz) → 8 ms (120 Hz) after CORPLAP-1
...
async fn handle_files_chunk(
    ...
    // the upload spinner spins forever — field repro CORPLAP-1 rc.8
    // (2026-05-06).
