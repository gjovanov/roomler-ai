...
/// seconds then re-sharpens" pulse (2026-07-25, NEO16 viewing CORPLAP-1 /
...
/// rc.219 — vp9_qsv-only SHORT GOP. Field-proven (2026-07-24, CORPLAP-1 Iris
...
fn encoder_options(
    ...
    if name.contains("nvenc") {
        ...
        // NEO16, hevc_nvenc; hevc_qsv flags forced-I correctly,
        ...
        // Field 2026-07-24 (NEO16 viewing CORPLAP-1): vp9_qsv IGNORED runtime
...
impl FfmpegEncoder {
    ...
    fn new_with_dispatch(
        ...
        for name in names {
            match Self::build_encoder(name, width, height, fps, maxrate_bps, cq, chroma444) {
                Ok(encoder) => {
                    tracing::info!(
                        encoder = name,
                        width,
    ...
    fn build_encoder(
        ...
        let (base, lowlat, opt_summary) = encoder_options(name, maxrate_bps, cq, chroma444);
