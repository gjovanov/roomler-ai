//! FFmpeg-based encoder backend (rc.67 onwards).
//!
...
//!
//! ## Phased rollout (Option B plan, shifted +1 after rc.65 CI failure)
//!
//! - rc.64: module declared, `available()` returns false. No FFmpeg
//!   link, no CI plumbing. Safe back-out window.
//! - rc.65: attempted to wire ffmpeg-next + vcpkg ffmpeg, CI FAILED at
//!   vcpkg install (baseline shipped ffmpeg 6.1 instead of 7+, and
//!   `avutil` is never a selectable feature in the port).
//! - **rc.66 (this RC)**: hotfixes the rc.65 vcpkg manifest. Bumps
//!   baseline to vcpkg HEAD (ffmpeg 8.1.1, matching ffmpeg-next 8.1)
//!   and drops `avutil` from the feature list. CI Windows job now
//!   installs FFmpeg cleanly + links libavcodec into the MSI.
//!   `linked_libavcodec_version()` exercises a symbol from the dep so
//!   the linker can't dead-strip the FFmpeg lib. Test asserts the
//!   version constant is in the FFmpeg 7+ range, locking the
//!   binding ↔ FFmpeg version pair.
//! - rc.67: `FfmpegEncoder` implements `VideoEncoder` trait. D3D11VA
//!   zero-copy from day 1 (critique fix). Behind
//!   `ROOMLER_AGENT_USE_FFMPEG=1` env var; MF cascade still default.
//! - rc.68: `caps::detect` advertises `data-channel-hevc`; new
//!   `hevc_dc_framer.rs` reuses 13-byte header from VP9-444 path.
//!   Anti-IDR-storm coalescer included (critique fix).
//! - rc.69: browser HEVC worker (pre-flight spike confirmed Annex-B
//!   no-description round-trip).
//! - rc.70: `vp9_qsv` HW path for Intel iGPU.
//! - rc.71: single codec-selector dropdown.
//! - rc.72+: AIMD tuning + field hotfixes.
//! - rc.75: Linux VAAPI.
//! - rc.76: macOS VideoToolbox.
//!
...
/// link time, not at the rc.67 encoder construction time.
///
...
/// rc.67 placeholder. Returns `false` until the real `FfmpegEncoder`
...
mod tests {
    ...
    #[test]
    fn available_returns_false_until_rc67() {
        assert!(
            !available(),
            "rc.66 ships only the link probe; rc.67 flips this to true"
        );
    }
