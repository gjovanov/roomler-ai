...
async fn main() -> Result<()> {
    ...
    // clicked. Field bug CORPLAP-1 2026-05-01. Idempotent — a noop once
    ...
    #[cfg(target_os = "windows")]
    {
        tracing::info!(
            ...
            "DPI awareness configured at process start (rc.41 diagnostic — surfaces residual CORPLAP-1 mouse-misposition cause)"
        );
        // rc.48 — monitor-layout diagnostic. DPI is correctly set per
        // the rc.41/44 readback, yet CORPLAP-1 field reports still show
    ...
    let config_path = match cli.config.clone() {
        ...
        None => {
            ...
            // enrollment config actually lives. Field repro CORPLAP-1
    ...
    // rate-limit. Field repro 2026-05-17 CORPLAP-2: the SystemContext
...
async fn run_cmd(config_path: &PathBuf, cli_encoder: Option<&str>) -> Result<()> {
    ...
    tokio::select! {
        res = sig_task => {
            ...
            // increment. M5 finding #2 (CORPLAP-1 2026-05-02): every
