...
impl OwnedProcess {
    ...
    /// spawned. M5 finding #8 (CORPLAP-1 2026-05-02): a 145 ms gap
...
/// 145 ms gap on CORPLAP-1 and well under any human-perceptible
...
/// Field repro CORPLAP-1 2026-05-06 showed that the swap window
    ...
    loop {
        ...
        match (decision, due_for_respawn) {
            ...
            (SpawnDecision::Idle, _) if current_worker.is_some() => {
                ...
                // 2026-05-01 (CORPLAP-1): logout → flood of "Zugriff
...
mod tests {
    ...
    fn decide_spawn_idles_when_session_disappears_without_active_peer() {
        // Field bug CORPLAP-1 2026-05-01: user logs out; active session
