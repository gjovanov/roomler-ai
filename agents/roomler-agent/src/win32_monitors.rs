...
//! reject (3) against CORPLAP-1 field data BEFORE writing a fix. No
...
pub fn log_monitor_diagnostic() {
        ...
        && (p.origin_x != 0 || p.origin_y != 0)
    {
        tracing::warn!(
            origin_x = p.origin_x,
            origin_y = p.origin_y,
            "monitor_diag — primary monitor origin is NOT (0,0); to_pixels currently maps against primary dims without applying this offset. Likely root cause of CORPLAP-1 mouse-offset bug."
        );
    }
}
