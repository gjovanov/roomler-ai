...
/// 2s with `code=1`, field repro 2026-05-17 CORPLAP-2) would otherwise
...
/// died). Field repro: CORPLAP-2 SystemContext loop, 2026-05-17.
///
...
pub fn record_with_log_tail(
    ...
    match outcome {
        Ok(Ok(path)) => {
            // Downgraded INFO → DEBUG (2026-05-17 CORPLAP-2 storm fix):
...
fn record_inner(
    ...
    // Crash-loop suppression. Field repro 2026-05-17 on CORPLAP-2:
...
mod tests {
    ...
    // ─── Crash-loop suppression (2026-05-17 CORPLAP-2 storm fix) ─────────────
