...
/// `0.4.<counter>` (the `0.3.0-rc.N` scheme is retired; `agent-v0.3.0-rc.484`
/// is the last rc). The counter continues past the last rc number, so MSI
/// ProductVersions stay monotonic across the transition
/// (`0.3.484 → 0.4.485 → 0.4.486 …`), and the version binding below becomes
/// EXACT for finals instead of minor-granular.
///
...
mod tests {
    ...
    fn final_releases_map_exactly_for_the_0_4_scheme() {
        ...
        // field (`0.4.485`, `0.4.486`, …), so the MSI ProductVersion is the
        ...
        assert_eq!(
            msi_product_version_for("agent-v0.4.485").as_deref(),
            Some("0.4.485")
        );
        assert_eq!(msi_product_version_for("1.2.3").as_deref(), Some("1.2.3"));
        // Monotonic across the transition: the last rc (0.3.484) < the first
        // 0.4 final (0.4.485) both as semver and as MSI ProductVersion.
        assert_eq!(
