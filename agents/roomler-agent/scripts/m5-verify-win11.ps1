...
reversible (-Action Rollback). Runs on CORPLAP-1 (the dev box) but
...
function Do-Install {
    ...
    # field repro on CORPLAP-1 2026-05-02 -- both were registered.
    Require-Elevated
    Require-AgentExe
    ...
    Write-Host '     connect to CORPLAP-1. Confirm video + mouse + keyboard work.'
    Write-Host ''
    Write-Host '  2. While streaming, log out of CORPLAP-1. Expected:'
