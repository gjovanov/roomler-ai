...
The three rules that make this safe, each learned the hard way:
  ...
  * LONGEST KEY FIRST.  A `DESKTOP-`/`LAPTOP-` qualified hostname has to be
    ...
    `<alias>`.
  * WHOLE WORD ONLY.  This is what makes a 3-letter shorthand like `CLK` safe
    to map at all.  Verified against every blob in this repo's history: all
    1145 bare `clk`/`CLK` occurrences are prose, not one is a code identifier.
...
def load_map(path):
    ...
    purpose, so mapping the hostname `GORAN` makes it shout about every
    ...
    # Case-SENSITIVE, because the whole point of the directive is that GORAN
...
def residual_scan(text, pairs, safe, keep=frozenset()):
    ...
    today, but `corplap-01 -> corplap-3` was one edit away from it), and case
    does not matter for those. `keep` is matched case-SENSITIVELY, because
    `GORAN` the hostname is rewritten while `goran` the profile directory is
