...
The three rules that make this safe, each learned the hard way:
  ...
  * LONGEST KEY FIRST.  A `DESKTOP-`/`LAPTOP-` qualified hostname has to be
    ...
    `<alias>`.
  * WHOLE WORD ONLY.  This is what makes a three-letter shorthand safe to map
    at all, and it has to be VERIFIED rather than assumed: dump every blob in
    history and read the contexts (recipe in SKILL.md).  For the one such key
    in this map, all 1145 occurrences turned out to be prose and not one was a
    code identifier -- but that was a measurement, not a guess.
...
def load_map(path):
    ...
    residue -- needed because the residual scan is case-insensitive on purpose,
    so a hostname that happens to be someone's first name makes it shout about
    every `C:\\Users\\<name>` path and every `<name>@...` author address.
    Without the directive the only ways to silence that are to map the
    lowercase form (which would rewrite the author identity of every commit in
    the repo) or to stop trusting the residual line -- and the residual line is
    the whole reason the 2026-08-28 sweep's 60 missed tags were ever found.
    ...
    # Case-SENSITIVE, because the whole point of the directive is that the
    # uppercase spelling is a rewrite key while the lowercase one is kept --
    # they differ only in case. A !keep of the EXACT spelling of a rewrite key
    # would silently disarm that rule's residual check, which is what this
    # rejects.
...
def residual_scan(text, pairs, safe, keep=frozenset()):
    ...
    today, but a retired alias folding onto its successor was one edit away
    from it), and case does not matter for those. `keep` is matched
    case-SENSITIVELY, because the uppercase spelling of a `!keep` token is a
    hostname that IS rewritten while the lowercase one is a profile directory
    that is not, and the two differ only in case.
    """
