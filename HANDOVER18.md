...
> session. Two field bugs surfaced on CORPLAP-1 that need a clean
...
- **CORPLAP-1 on**: `roomler-agent 0.3.0-rc.21` (manually installed via perMachine MSI)
- **Active user on CORPLAP-1**: `e069019` (NOT `corpuser` as initially assumed — that's a different account on the same box)
...
For e069019 on CORPLAP-1 specifically: `active_user_downloads_path()` resolves to `C:\Users\e069019\Downloads` (his Downloads is NOT redirected — only `corpuser`'s was), so the PROGRAMDATA fallback isn't reached. Small uploads land successfully at `C:\Users\e069019\Downloads\<file>` as expected.

## Open Bug 1: Large file uploads still fail on CORPLAP-1
...
**Working hypothesis** (REVISED 2026-05-12): **ESET Security real-time scanner** intercepts the staging `data` file writes. CORPLAP-1 has `C:\Program Files\ESET\ESET Security\ekrn.exe` running — the user's box is an ESET-protected corporate Win11, NOT Defender. Initial Defender-exclusion test was a misfire (`Add-MpPreference` adds Defender exclusion; doesn't affect ESET). User-side test confirmed: large .zip still fails after Defender exclusion was added.

CORPLAP-1 ESET stack:
...
The box is also heavily managed: SCCM (`C:\Windows\ccm`, `ccmsetup`), Tanium endpoint client (`C:\Program Files (x86)\Tanium\Tanium Client`), Dameware remote support. Multiple users on the same machine (`e069019`, `corpuser`, `f002459`, `sichern2`, `Werkstatt`). Adding ESET exclusions probably requires the central ESET Remote Admin console; can't be done locally.
...
2. Install each in sequence on CORPLAP-1 (with rc.20 MSI cleanup-legacy-install handling cross-flavour transitions cleanly).
...
2. **30 min** — bisect rc.7 → rc.21 for admin-pwsh-input regression on CORPLAP-1.
...
4. **30 min** — manual smoke on CORPLAP-1 + tag rc.22.
...
2. Have user run the Defender exclusion test on CORPLAP-1 (5 min).
...
4. The user's CORPLAP-1 admin pwsh setup is the canonical reproduction. Fast iteration possible since they're actively testing.
