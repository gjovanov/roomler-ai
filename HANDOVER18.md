...
- **Active user on CORPLAP-1**: `e069019` (NOT `extjovanov` as initially assumed — that's a different account on the same box)
...
For e069019 on CORPLAP-1 specifically: `active_user_downloads_path()` resolves to `C:\Users\e069019\Downloads` (his Downloads is NOT redirected — only `extjovanov`'s was), so the PROGRAMDATA fallback isn't reached. Small uploads land successfully at `C:\Users\e069019\Downloads\<file>` as expected.
