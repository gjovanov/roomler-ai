...
- [MEDIUM] [2026-08-20] **A hardware-encoder caps probe can kill the daemon.** The FFmpeg HW probe runs IN-PROCESS at startup, so a segfault inside a vendor driver takes `roomlerd` down — and systemd restarts it straight back into the same probe, producing a crash-loop rather than a degraded-but-running agent. Observed on the WSL sibling: WSL ships `/usr/lib/wsl/lib/libcuda.so.1` as a stub with no usable driver, so `hevc_nvenc` **dlopens successfully** and then SEGVs when `cuInit(0)` fails — the "loaded but unusable" state. NOT version-specific (rc.421 and rc.424 both) and NOT a regression from any recent work; it is latent because the probe only runs at startup, so a long-running agent never re-enters it. Hosts with **no** libcuda (mars/zeus/jupiter: `ldconfig -p | grep -c libcuda` = 0) take the clean dlopen-failed branch and are unaffected. ⚠️ `ROOMLER_AGENT_HW_AUTO=0` / `ROOMLER_AGENT_ENCODER=software` do NOT skip it — those pick an encoder; the caps probe enumerates what to ADVERTISE and always runs. **Fix direction: run the HW probe in a CHILD PROCESS and treat "child died" as "codec unavailable".** A capability probe is untrusted-third-party code by definition (vendor drivers, GPU firmware) and should not share an address space with the daemon; that also removes the whole class rather than the one CUDA symptom. Detail + the disproof of the "it was the new release" theory in the session notes.
