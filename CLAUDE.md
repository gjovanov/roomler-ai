...
agents/
  ...
  roomler-tunnel/   → Tunnel client. Bin `roomler` (tunnel-only hosts); the whole command surface lives in the LIB (`cli.rs`) so the daemon can host it too
...
- TTL indexes on audit_logs (90 days), activation_codes, background_tasks, **remote_audit (90 days)**, **exec_audit (90 days)**, **ssh_audit (90 days)**
...
- [MEDIUM] [2026-08-21] **Windows: a redundant worker respawns in a hot loop.** `win_service::supervisor::decide_exit_reaction` maps exit `0` to `ExitReaction::Respawn` with **zero backoff**. When a second daemon flavour already holds the single-instance lock (perMachine service + perUser task both live), the worker exits 0 on the lock check and the supervisor relaunches it immediately, forever — the Linux bug fixed in rc.439, in a worse shape (no 5 s spacing). Linux got `ALREADY_RUNNING_EXIT_CODE = 8` + `RestartPreventExitStatus`, but that constant is deliberately **not** emitted on Windows: a non-zero code there lands in the `else` arm, so the worker would respawn on the escalating backoff ladder *and* `should_record_supervisor_crash` would write a crash sidecar for what is a correct refusal. The fix is a new `ExitReaction` (stop, don't respawn) plus a `should_record_supervisor_crash` exclusion — not a code renumber. No field sighting yet (the MSI's cross-flavour cleanup makes the two-flavour state rare); found by reading the supervisor while fixing the Linux side.
