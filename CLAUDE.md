...
- [MEDIUM] [2026-08-03] Overlay ACL is **control-plane only**. `overlay_policies` + `OverlayNetwork.acl_mode` (`off`|`warn`|`enforce`, default `off`) now shape the netmap per recipient and gate the TURN relay-grant path, which constrains an HONEST client. Two tiers are still open against a modified/stale one: (a) the advertising node applies **no ingress filter** — it writes any decrypted packet to the TUN regardless of destination, and there is no reverse-path check at all, so a peer with a live WG session can inject any source/destination; (b) `ws::derp::forward_frame` is scoped by `network_id` only, so a denied pair that already holds each other's pubkeys can relay around the netmap. Both need the node-side filter (+ a cached per-network allow-set for DERP) — see `crates/api/src/ws/derp.rs` for the intended design. Port/proto on a rule are stored and distributed but **not yet enforced**, because the netmap wire format has no port dimension.
