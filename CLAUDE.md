...
- **Tenant-affinity LB** (S6): the front reverse proxy (docker-nginx on the build host) routes to the pods through a `hash <tenant-key> consistent` upstream — key = path tenant (`/api/tenant/{tid}/...`) → `?tid=` query param → client IP. This co-locates a tenant's users, agents, tunnel clients, DERP sockets and mediasoup rooms on ONE pod (the rc-hub / tunnel-hub / DERP relay / room registry are pod-local). The second pod ships parked `down` in the upstream until the agent fleet runs a `tid`-carrying build (≥0.3.0-rc.264) — flipping earlier would split tid-less agents from their controllers across pods. Cross-pod chat/notifications/presence ride the Redis fan-out; a Redis online-registry (`roomler:online:<uid>`, 90 s TTL + 30 s heartbeat) backs the offline push/email dedupe; startup maintenance (stale-call reset, migrations) is leader-gated behind a 120 s Mongo lease (`locks` collection).
...
**Design + architecture**: `docs/remote-control.md` (16 sections covering goals, topology, protocol, data model, security, latency budget). Overlay-mesh sub-topics: `docs/agent-tunnel-architecture.md`, `docs/overlay-wfp.md` (Windows firewall override), **`docs/overlay-exit-nodes.md`** (Tailscale-style exit nodes), **`docs/overlay-nat-traversal.md`** (carrier-selection mechanics: LAN→direct-public→srflx hole-punch→single-relay→DERP), **`docs/overlay-communication.md`** (the end-to-end picture with mermaid diagrams: control plane vs data plane, every tier as a sequence diagram, and which carrier wins inside vs outside a corporate VPN — start here).
...
- nginx security headers: X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy (2026-03-21); HSTS + CSP added 2026-07-28 (`files/nginx-pod.conf` — CSP rationale in the comment: no third-party scripts, 'unsafe-inline' styles for Vuetify, https: images for Giphy/avatars).
...
- CVE scan baseline (2026-07-28, first run): **`bun audit`** — one CRITICAL cleared by a compatible `bun update` (vitest UI-server arbitrary-file-read, dev-only), leaving 45 (16 high / 22 mod / 7 low), ALL in dev/transitive deps (js-cookie via @vue/test-utils, linkify-it via markdown-it, rollup/undici/minimatch) with no runtime-reachable server path. **`cargo audit`** — 9 advisories, all transitive: rsa 0.7.2 Marvin timing sidechannel (via mongodb SCRAM — not attacker-triggerable over the wire), quinn-proto/quick-xml/lopdf memory-exhaustion-on-malicious-input (DoS-class), idna/crossbeam/memmap2/spin unsound-or-yanked. None warrant an emergency bump; a scoped dependency-uplift pass is the follow-up. Re-run: `bun audit` (ui) + `cargo audit` (mars, `~/.cargo/bin`).
