...
**Resumption note after a session break**: `HANDOVER9.md` is the most recent (covers Tier A + viewer controls + in-field fixes in 0.1.33 → 0.1.35). `HANDOVER2.md` through `HANDOVER8.md` trace the historical arc. **For the current resilience-cycle state (0.1.50 → 0.1.54), read `docs/remote-control.md` §19 — the agent now self-heals from crashes, verifies update integrity, auto-rolls-back on broken releases, and registers its Scheduled Task at MSI install time without operator intervention.**
