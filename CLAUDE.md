...
- [MEDIUM] [2026-08-03] Overlay ACL does not yet constrain a **modified** client. `overlay_policies` + `OverlayNetwork.acl_mode` (`off`|`warn`|`enforce`, default `off`) shape the netmap per recipient and gate the TURN relay-grant path, which constrains an HONEST one; `overlay_rpf` (default `warn`) adds node-side **reverse-path** validation, so a peer can no longer forge a SOURCE address it doesn't own. Still open: (a) there is no **destination** filter — the advertising node writes any decrypted packet to the TUN regardless of where it's addressed, so a peer that reaches a subnet router at all reaches everything behind it; (b) `ws::derp::forward_frame` is scoped by `network_id` only, so a denied pair that already holds each other's pubkeys can relay around the netmap. (a) needs an L4 packet parser + server-compiled per-peer rules on `NetmapPeer` (the `Ingress` struct in `overlay/wg.rs` is the seam — it already carries peer identity to every inbound call site); (b) needs a cached per-network allow-set — see `crates/api/src/ws/derp.rs` for the intended design. Port/proto on a rule are stored and distributed but **not yet enforced**, because the netmap wire format has no port dimension.
