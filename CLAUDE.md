...
**Resumption note after a session break**: `HANDOVER17.md` is the most recent — rc.19 (resumable file-DC transfers) shipped on both agent + browser sides, `agent-v0.3.0-rc.19` tag pushed, release workflow building MSIs. K8s web deploy + PC50045 P8 smoke are the remaining items. `HANDOVER16.md` captures the mid-cycle state (agent shipped, browser pending). `HANDOVER15.md` covers the rc.16 → rc.18 cycle. `HANDOVER2.md` → `HANDOVER14.md` trace the historical arc. **For the current resilience-cycle state (0.1.50 → 0.1.54), read `docs/remote-control.md` §19 — the agent now self-heals from crashes, verifies update integrity, auto-rolls-back on broken releases, and registers its Scheduled Task at MSI install time without operator intervention.**
