...
- [MEDIUM] [2026-08-03] Overlay ACL enforces **coarsely** on the node, not per-source. `overlay_policies` + `OverlayNetwork.acl_mode` (`off`|`warn`|`enforce`, default `off`) shape the netmap per recipient and gate BOTH relay tiers (TURN relay-grant, and `ws::derp_acl`'s precomputed per-network allow table). `overlay_rpf` (default `warn`) adds the node-side data plane: a peer can neither forge a SOURCE it doesn't own nor address a DESTINATION outside the subnets this node advertises. What remains is **per-source narrowing**: every peer that may reach a subnet router may reach *everything* that router advertises, on every port — a policy saying "only Alice, only 10.66.0.0/24, only :22" is enforced only as far as "10.66.0.0/16". Closing it needs an L4 packet parser (none exists — `Router::dst_of_ip_packet` reads destination only, and `Cidr` is v4-only) plus server-compiled per-peer rules on `NetmapPeer` (⚠️ 14 struct-literal sites + 13 serde wire-lock tests ⇒ a new field must be `#[serde(default, skip_serializing_if)]`). The seam is `Ingress` in `overlay/wg.rs`, which already carries peer identity to all three inbound call sites. Port/proto on a rule are stored and distributed but **not yet enforced** for the same reason.
