...
**Resumption note after a session break**: `HANDOVER18.md` is the most recent — captures the rc.20 + rc.21 ship state and two OPEN field bugs on CORPLAP-1: (1) large-file uploads still failing post-rc.21 (Defender-on-staging hypothesis, untested), (2) mouse/input fails over admin-pwsh windows (regression between rc.7 and rc.21). `HANDOVER17.md` captures the rc.19 ship. `HANDOVER15.md` → `HANDOVER16.md` cover the rc.16 → rc.19 cycle. `HANDOVER2.md` → `HANDOVER14.md` trace the historical arc. **For the current resilience-cycle state (0.1.50 → 0.1.54), read `docs/remote-control.md` §19 — the agent now self-heals from crashes, verifies update integrity, auto-rolls-back on broken releases, and registers its Scheduled Task at MSI install time without operator intervention.**
...
**Status at 0.3.0-rc.22** (ESET-evasive staging strategy on Windows, shipped 2026-05-12): rc.21's `download_dir()` fix landed PROGRAMDATA fallback only when `active_user_downloads_path()` resolved to None (Folder-Redirected user case). Field repro on CORPLAP-1 2026-05-11 showed large-file uploads (14 MB .exe / 35 MB .xlsx) still failed with "reconnect budget exhausted after 6 attempts" on the e069019 account whose Downloads is NOT redirected — staging hit `C:\Users\e069019\Downloads\.roomler-partial\<id>\` where **ESET Security** real-time scanner (the corporate AV stack — `ekrn.exe`, `efwd.exe`, `ERAAgent.exe`) hooks per-chunk writes aggressively enough to push the file-DC's SCTP buffer into overflow. rc.22 moves staging on Windows to `%PROGRAMDATA%\roomler\roomler-agent\staging\<id>\` ALWAYS (no longer per-dest), keeping the per-chunk write loop on a path corporate AVs typically scan less aggressively, then crossing into the user's Downloads only once at end-time rename. Cross-volume rename fallback via `tokio::fs::copy` + `remove_file` for cases where Downloads lives on a different volume. Env-var escape hatch `ROOMLER_AGENT_STAGING_LEGACY_PER_DEST=1` reverts to rc.19 layout without rebuild. Hypothesis-driven; if rc.22 still fails on CORPLAP-1, falls through to Option D (request corporate ESET exclusion via central admin console). New helpers: `STAGE_IN_PROGRAMDATA: LazyLock<bool>` (defaults true on Windows, false on other / cfg(test)), `staging_root_windows()`, `is_cross_volume_error(&io::Error)`. `sweep_orphans()` refactored into a shared `sweep_orphans_dir()` body that handles both the legacy `<root>/.roomler-partial/` layout and the new flat `<root>/<id>/` PROGRAMDATA layout. 6 new tests (strategy flag, partial_dir_for under cfg(test), 3 cross-volume-error classification cases, staging_root_windows shape, flat sweep). 285 agent-lib tests green. The M3 elevated/user-app input regression (Bug 2 on HANDOVER18 — admin pwsh input failing on rc.21) is NOT addressed in rc.22 — separate plan landed in `docs/remote-control-m3-elevated-switching.md` for the next cycle.

**Status at 0.3.0-rc.20** (rc.19 hotfix bundle, shipped 2026-05-11): two field-blocker fixes on top of rc.19. (1) `agents/roomler-agent-tray/Cargo.toml` adds `"custom-protocol"` to the `tauri` features so `cargo build --release` produces a self-contained EXE with embedded HTML/CSS/JS; without it the release tray loaded `devUrl: localhost:1430` at runtime → "ERR_CONNECTION_REFUSED" in the webview. `tauri.conf.json::frontendDist` corrected from `"../src/front"` (resolved to a non-existent `agents/src/front`) to `"src/front"` (the actual layout). (2) `chunk_syncs_data_at_one_mib_boundary` test no longer asserts on `tokio::fs::metadata().len()` — that's tokio's internal write-buffer state, not the durability contract; the test now asserts on `last_synced` + `received` which IS the B2 invariant. CI run 25678861198 was failing on this assertion under Linux's slower buffer-drain cadence.

**Status at 0.3.0-rc.19** (resumable file-DC transfers — agent + browser shipped 2026-05-11):
- **Wire format** (P0): new `FilesIncoming::Resume { id, offset, sha256_prefix }` + `FilesOutgoing::Resumed { id, accepted_offset }` variants in `agents/roomler-agent/src/files.rs`. Caps add `"resume"` to `caps.files`; browsers opt in only when supported. `PARTIAL_REGISTRY: LazyLock<Mutex<HashMap<id, meta_path>>>` and `ACTIVE_TRANSFERS: AtomicUsize` globals.
- **Agent staging** (P1): every upload writes to `<dest_dir>/.roomler-partial/<id>/data` with a sibling `meta.json`. `chunk()` calls `sync_data()` per 1 MiB (B2 fix — tuned for Windows Defender, ~35-1050 ms overhead on a 35 MB upload). `begin()` rejects duplicate ids on disk (B1 fix). `sweep_orphans()` runs synchronously at agent startup BEFORE the WS signaling task spawns; walks `Downloads/.roomler-partial/`, deletes >24h dirs, registers survivors. `end()` re-runs `unique_path()` at rename time so operator file-ops mid-upload don't clobber the final (M4 fix). Same-volume rename → O(1).
- **Resume handler** (P2): `FilesHandler::resume_incoming(id, offset)` looks up the partial via `PARTIAL_REGISTRY` (on-demand stat fallback), validates `meta.json`, truncates the staging `data` file to `min(offset, disk_size) & !(256 KiB - 1)` (B3 fix — alignment matches `files:progress` cadence), reopens for append, reinstalls state. Cancel arm extended to upload-side — removes the per-id staging dir + registry entry so browser-driven terminal failures clean up immediately.
...
- **Browser auto-resume** (P4 + P5): `useRemoteControl(agent?: Ref<Agent>)` computes `supportsResume` from `capabilities.files.includes("resume")`. `RemoteControl.vue` passes its existing `agent` ref. `uploadOne` refactored into `innerPump` (re-reads live `channels.files` per iteration so reconnect doesn't capture the dead channel) + outer wrapper with a 6-attempt retry loop matching `RC_RECONNECT_LADDER_MS.length`. Attempt 0 sends `files:begin`; subsequent attempts await `phase === 'connected'`, send `files:resume`, await `files:resumed` via `pendingResumePromises` (separate Map mirroring `pendingDirRequests` shape), re-pump from `accepted_offset`. Budget exhaust sends `files:cancel` so the agent cleans staging immediately. UploadEntry gains `bytesAcked` updated on every `files:progress`. DC-close handler defers settling UPLOAD entries when `supportsResume` so resume can replay them; downloads still fail-fast.
- **UI polish** (P7): Transfers panel `v-if` extends to `phase === 'reconnecting'` (was invisible during retry, P1-5 fix). New `'reconnecting'` TransferStatus pill: warning colour, "Reconnecting — attempt N/6" label. Cancel button stays available so operator can abort mid-retry. `transfersInFlightCount` counts reconnecting transfers.
- **Tests**: 272 agent lib tests green (+15 new — 4 serde locks + 5 P1 staging + 5 P2 resume + 4 P3 defer + 1 caps). 13 file_dc integration tests (+1 — `files:resume unknown id → files:error`). 397 frontend Vitest (130 in useRemoteControl.spec). Clippy + fmt + vue-tsc all clean. The DC-recreate full-resume integration test was prototyped but deferred: webrtc-rs loopback SCTP teardown races the second DC's chunk loop on Windows. Lib tests cover the mechanics by driving `FilesHandler::resume_incoming` directly.
- **Open**: P8 (manual smoke on CORPLAP-1 — kill at 4/25/50/90%, auto-update mid-flight, sync_data perf re-measure, cross-version compat). v2 nice-to-haves: per-chunk SHA256 verification, download-resume (host → browser), sidecar registry at `%LOCALAPPDATA%\roomler\partial-index\` so non-Downloads dest_path uploads survive agent restart.

**Status at 0.2.6** (auto-update asset-picker fix shipped 2026-05-02 on top of 0.2.5):
- **Asset-picker now flavour-aware**. Pre-0.2.6 `pick_asset_for_platform` returned the first `.msi` in the GitHub release asset listing — alphabetical order which puts `…-perMachine-…msi` ahead of the plain `…-x86_64-…msi`. A perUser agent on 0.2.0 polling the 0.2.5 release downloaded the perMachine MSI, msiexec ran it `/qn`, the cross-flavour launch condition silently rejected the install, the parent agent exited expecting the installer to win, the Scheduled Task respawned the same 0.2.0 binary — auto-update made zero forward progress for any agent in the field. Field repro: CORPLAP-1 (e069019l) 2026-05-02. **Fix**: new `WindowsInstallFlavour` enum + `current_install_flavour()` reads `std::env::current_exe()` and classifies as `PerMachine` if under `\Program Files (x86)?\…`, else `PerUser`. New `pick_asset_for_windows(assets, flavour)` filters MSIs by `-perMachine-` infix, falls back to "any MSI" only if the matching flavour is missing. 7 new tests pin the contract (perUser skips perMachine even when alphabetically first; flavour-discovery from typical Windows install paths; case-insensitive; both fallback directions).
